...
- 【**字段列表**】中的【**编号**】必须是唯一的，不能重复，其将被用于识别字段的 Delta 变更。
  ...
  按字段添加的先后顺序进行递增，注意，不是按相对位置递增，在删除字段后，其余字段的编号保持不变。
  <u>注：【关联条件】下的【序号】列的数字不要求连续和唯一性，其仅用于标识有效行（其他表单中的【序号】列都是如此）</u>
...
- 【**关联列表**】中的【**属性名**】为必填项，其内容为当前对象与被关联对象绑定时的属性名称（命名规范自定），
  ...
  join 的字段所映射的对象属性名称，例如，连接条件为 `JOIN c.PARENT_ID = p.ID`，
  则左右属性分别为 `parentId` 和 `id`
