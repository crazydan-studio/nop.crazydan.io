...
| graphql:connectionProp | prop-name | 当前 Connection 属性所关联的属性名 | 否 | 详细说明见 [Connection 属性](#relations-connection) |
| graphql:queryMethod | `dict:core/std-query-method`/`GraphQLQueryMethod` | 被调用的过滤函数名 | 否 | 详细说明见 [Connection 属性](#relations-connection) |
| |
| transformIn | xpl |  | 否 |  |
| transformOut | xpl |  | 否 |  |
| getter | xpl |  | 否 |  |
...
### Connection 属性 {#relations-connection}

> 详细的说明文件见 [connection 配置](https://gitee.com/canonical-entropy/nop-entropy/blob/master/docs/dev-guide/graphql/connection.md)。
