...
<meta>
  <props>
    ...
    <prop name="relatedRightIdList" ext:relation="relatedRightList">
      <schema type="List&lt;java.lang.Integer&gt;"/>
    </prop>
    <prop name="relatedRightList" ext:kind="to-many">
      <schema>
...
> - `ext:rightJoinProp` 设置为在 `Ref`（关联的目标端）中用于与 `Left`（关联的源端）建立关联的属性；
> - `ext:leftJoinProp` 设置为在 `Ref` 中对应的 `ext:rightJoinProp` 所指向的 `Left` 的属性；
...
<meta>
  <props>
    ...
    <prop name="relatedLeftIdList" ext:relation="relatedLeftList">
      <schema type="List&lt;java.lang.Integer&gt;"/>
    </prop>
    <prop name="relatedLeftList" ext:kind="to-many">
      <schema>
...
> - `ext:kind` 设置为 `to-many`（一对多）模式关联 `Ref`；
> - `ext:rightJoinProp` 设置为在 `Ref`（关联的目标端）中用于与 `Right`（关联的源端）建立关联的属性；
> - `ext:leftJoinProp` 设置为在 `Ref` 中对应的 `ext:rightJoinProp` 所指向的 `Right` 的属性；
...
> - `ext:joinLeftProp` 表示在 `Ref`（关联的源端）中用于与 `Left` 或 `Right`（关联的目标端）建立关联的属性；
> - `ext:joinRightProp` 表示在在 `Ref` 中对应的 `ext:joinLeftProp` 所指向的 `Left` 或 `Right` 的属性；
> - `ext:relation` 用在 `Ref`（关联的源端）直接与关联目标端建立关联的属性上，
>   其指向在 `Ref` 中对应的关联目标端对象类型的属性上，如，`leftId -> left`、`rightId -> right`；
