...
| graphql:queryMethod | `dict:core/std-query-method`/`GraphQLQueryMethod` | 被调用的过滤函数名 | 否 | 详细说明见 [XMeta 对象关联配置 - 关联过滤查询](#relations-connection) |
| graphql:connectionProp | prop-name | 当前 Connection 属性所关联的属性名 | 否 | 详细说明见 [XMeta 对象关联配置 - 关联过滤查询](#relations-connection) |
...
则只需要在 `NopAuthSite.xmeta` 中为其对象属性 `resourcesList` 设置 `graphql:queryMethod`：
