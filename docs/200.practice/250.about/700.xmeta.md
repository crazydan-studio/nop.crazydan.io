...
| graphql:transFilter | `xpl-fn:(filter, query, forEntity) => any` | 自定义过滤器 | 否 | 用于将针对当前对象的过滤器进行命名，以便于通过名称引用并附加该过滤器。详细说明见 [XMeta 过滤器命名](#attr_props_prop_graphql_transFilter) |
...
## XMeta 过滤器命名 {#attr_props_prop_graphql_transFilter}
