...
本章节还在整理中，当前内容仅用于备份留存。

:::

```graphql {5}
query ($query: QueryBeanInput) {
  DepartmentEntity_findList {
    value: oid
    label: displayName
...
通过 `GraphQLSelectionResolver#expandTreeChildren` 展开指定层数的选择字段：
...
展开后的 GraphQL 选择字段的总层数不能超过配置项
`nop.graphql.query.max-depth` 所设置的值，其缺省值为为 `7`。
也就是，指令 `@TreeChildren` 的参数 `max` 的最大值只能为配置项
`nop.graphql.query.max-depth` 的值减去该指令所在的层级数。

在应用了 `@TreeChildren` 指令的字段，不能再显式为其指定子字段，否则，NopGraphQL
将不会对其进行展开，该指令也就为失效状态。仅以下定义是有效的：
...
直接按指令 `@TreeChildren` 指定的层级展开选择字段是一种与
ORM 无关的处理方式，可以支持更多的取值场景，而不必限定在 ORM 框架之中。

在引入了 `nop-graphql-orm` 模块，并且在 XMeta 模型中配置了
[<entityName />](../../manual/xmeta#attr_entityName) 后，
便可以自动支持对子节点数据的拉取。

- 初始化 `io.nop.graphql.orm.OrmBizInitializer#initialize`；
- 构造对关联数据字段的取值函数 `io.nop.graphql.orm.OrmFetcherBuilder#buildFetcher`
  - 一对一采用 `OrmEntityRefFetcher`
  - 一对多采用 `OrmEntitySetFetcher`

若是未映射 ORM 实体，或者被展开的字段未映射 ORM 实体的关联属性，
则需要由该字段的取值函数直接返回指定层级的子节点数据，或者构造与
`OrmEntitySetFetcher` 相似的取值函数。
