...
- [GraphQL Argument](https://spec.graphql.org/October2021/#Argument):
  GraphQL 字段参数，用于控制 GraphQL Field 的取值结果，比如，按照参数指定的过滤条件查询数据。
...
比如，按照 `query` 参数中的过滤条件做数据查询。因此，不能简单地将 GraphQL
...
> 在 NopGraphQL 中分页查询结果 `PageBean<UserEntity>`
...
可以通过 `IObjMeta` 自动推导并构造出由 `GraphQLObjectDefinition`
模型描述的 GraphQL 对象类型，同时为其生成相应的字段定义 `GraphQLFieldDefinition` 列表，
...
`UserEntity`，而分隔符之后的部分对应的则是在 `IBizModel` 上定义的业务操作函数的名字，如
`findPage`，并且，操作函数只能是被标注为 GraphQL `query` 或 `mutation` 的函数。
...
> 本章节不会对其校验过程进行展开分析，可以按照上图中的代码调用链查看相关方法的具体实现来了解其校验逻辑。
...
对 `GraphQLDocument` 的取值逻辑可以简化为以下代码：
...
没有为其定义元数据。为此，NopGraphQL 对 `PageBean<?>` 做了专门的处理，为其单独构造了
...
最后，NopGraphQL 会以 `GraphQLDocument` 的取值结果构造 `GraphQLResponseBean`
后，再返回给客户端：
...
不会再对其结构做深度遍历。其仅在[初始化 GraphQLDocument](#engine-init-graphql-doc)
时，才会先根据该字段关联的业务对象名字（对应调用
`fieldDef.getType().getNamedTypeName()`）得到业务对象的 GraphQL 对象类型定义
`GraphQLObjectDefinition`，再根据 `GraphQLObjectDefinition` 的结构继续递归初始化子字段。

> 可以查看 `ObjMetaToGraphQLDefinition#toGraphQLType` 的实现以了解对关联业务对象的构造逻辑。
...
> - 操作函数可以直接返回 `CompletionStage` 类型的值，真正的取值，可以在子线程中异步进行；
...
会将同名的 XBiz 模型上定义的操作函数与强类型方式定义的操作函数进行合并，
并且，XBiz 中的操作函数优先级是最高的，在 Java 类中定义的同名操作函数会被直接覆盖掉。

XBiz 模型可以极大地增强业务系统的可扩展性，例如，在 xbiz 文件中，仅通过如下方式便可引入逻辑编排：
...
一般情况下，为了提升关联查询的性能，会采用批量加载机制。此时，可以直接将 `@ContextSource`
标注的参数定义为 `List<UserEntity>` 类型，NopGraphQL 将会自动采用
[DataLoader](https://www.graphql-java.com/documentation/batching/)
机制进行批量加载。

例如，在以下的 GraphQL Document 中有多个 `roles` 字段，其均会调用对应的 `@BizLoader` 函数：
...
`List<UserEntity>` 以参数形式传递给 `@BizLoader` 函数以批量加载 `RoleEntity`。
...
实例 `loader`，并将 `loader` 注册到 GraphQL 取值的上下文 `IGraphQLExecutionContext`
中以便于复用，最后，执行方法 `loader.load(...)` 以返回代表字段取值结果的
Promise 异步对象：
...
以下面的 GraphQL Document 字段取值为例（省去了无关的字段）：
...
字段取值函数 `BeanMethodBatchFetcher` 每次被调用时，都会调用相同实例的
`DataLoader` 的 `load(Object)` 方法，其会为入参构造唯一的 Promise 对象（即上图中的
...
也已经构造完成了，只是它们还在等待各自 `UserEntity#roles` 的取值结果。

NopGraphQL 是在方法 `GraphQLExecutor#executeAsync` 中进行 GraphQL Document
的字段取值的，查看该方法，可以发现，其在调用 `GraphQLExecutor#invokeOperations`
方法得到 $P_0$ 后，便会调用 `IGraphQLExecutionContext#dispatchAll` 方法。

而 `IGraphQLExecutionContext#dispatchAll` 会依次调用在
`IGraphQLExecutionContext` 中注册的 `DataLoader` 实例的 `dispatch()`
方法，并最终由 `DataLoaderHelper#dispatchQueueBatch`
...
后，作为参数传给 `BatchLoader#load` 方法，并进而得到执行
`UserEntityBizModel#roles` 后得到到的批量数据 `List<List<RoleEntity>>`。

接着，依次遍历 `List<List<RoleEntity>>` 得到相同位置的 `UserEntity#roles`
所关联的 `List<RoleEntity>`，再调用与各个 `UserEntity` 绑定的 Promise
对象（$P_{r0}, P_{r1}, P_{r2}, P_{r...}, P_{rn}$）的
`CompletableFuture#complete` 方法结束 Promise，
并同时将各自的字段取值结果传递给后续的链式调用函数。

也就是，`DataLoader` 会首先做全局的批处理参数的收集，并将其放在 List 列表中，
同时，为各个批处理参数构造并绑定唯一的 Promise 对象，这些 Promise
对象将一直等待各自的字段取值结果。

在批处理参数收集完毕后，再调用 `DataLoader#dispatch` 便会真正执行批量加载函数，
并将得到的批量数据按顺序传给各个 Promise 对象，从而继续后续的字段取值操作。
...
所以，需要避免在返回 Promise 的操作字段内使用批量加载字段。
