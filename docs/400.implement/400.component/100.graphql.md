...
- **GraphQL 字段取值函数**：在 GraphQL Field（GraphQL 字段）上绑定的用于获取字段值的函数；
...
传统的 GraphQL 在使用前，还必须为 GraphQL Document 定义相应的 GraphQL
Schema（GraphQL 模式），以用于校验客户端提交的 GraphQL Document 的有效性。而 NopGraphQL
则会根据 `IBizModel` 和 `IObjMeta` 自动推导并生成 GraphQL Schema，并不需要手工编写。
...
> `query` 函数由注解 `@BizQuery` 标注，而 `mutation` 函数则由注解 `@BizMutation` 标注。
...
和 `mutation` 函数收集起来，并构造为 GraphQL 操作字段，再单独存放到
`BizObjectImpl#operations` 中。

而在 `BizObjectImpl#objectDefinition` 对应的类型 `GraphQLObjectDefinition`
中，则仅仅包含映射到业务数据属性的 GraphQL 字段定义，并不包含操作字段，
因此，其与 `IObjMeta` 在形式上是等价的。

不过，`GraphQLDocument` 代表的是客户端提交的 GraphQL Document 的实例对象，
其主要作用是做字段取值，而操作字段与普通字段在取值方式上是一致的，所以，并不需要像
`BizObjectImpl` 一样明显区分二者。只是在初始化 `GraphQLDocument` 时，根字段的字段定义
`GraphQLFieldSelection#fieldDefinition` 需要单独从 `BizObjectImpl#operations`
中取出，而其他字段则直接根据字段名字从 `BizObjectImpl#objectDefinition` 中取出即可。

### 驱动逻辑 {#engine-impl-run}
