...
在 NopGraphQL 中，业务数据的结构由 `IObjMeta` 模型描述，其主要包含
`IObjPropMeta`（**业务对象属性元数据**）列表，因此，可以根据 `IObjPropMeta`
构造 GraphQL 对象类型的字段定义（由 `GraphQLFieldDefinition` 模型描述），
再由 `GraphQLFieldDefinition` 列表组装生成 GraphQL 对象类型（由
`GraphQLObjectDefinition` 模型描述），故而，`GraphQLObjectDefinition`
可以自动推导生成，并不需要手工编写 GraphQL Schema。

此外，在构造 `GraphQLFieldDefinition` 的过程中，还会将其与相应的字段取值函数 `IDataFetcher`
进行绑定，用以支持一些自定义的字段取值需求。

> 一般不需要为字段定义 `GraphQLFieldDefinition` 绑定取值函数 `IDataFetcher`，NopGraphQL
...
若不能为 GraphQL 字段找到其定义，则其必然不是有效的 GraphQL Document，直接抛出异常即可。

在沿着 GraphQL Document 字段树完成对 `GraphQLDocument` 的字段初始化后，
接下来，便可以对 `GraphQLDocument` 进行字段取值了。
