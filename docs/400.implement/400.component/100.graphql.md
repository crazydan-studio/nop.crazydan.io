...
作为返回结果的属性名。并且，在字段取值结果是列表类型，且列表元素为对象类型时，如
`items`，GraphQL 会遍历该列表，并对列表元素做字段选择，再将字段选择后的结果按遍历顺序组装到
GraphQL 返回结果中。
...
并且，通过字段取值函数，可以屏蔽不同字段的取值差异，统一字段取值逻辑，让
GraphQL 的字段取值更加灵活方便，也能够支持如批量加载、获取远程数据等复杂的取值需求。
