...
对 `GraphQLDocument` 取字段值的过程，就是沿着其 GraphQL Field Selection
构成的 GraphQL 字段树，挨个调用字段取值函数，并返回与字段树具有相同结构的取值结果的过程：
...
在 NopGraphQL 中，GraphQL 的取值结果将被放在 `GraphQLResponseBean#data` 属性上，
故而，以上 `GraphQLDocument` 返回到客户端的最终结果为如下形式：
