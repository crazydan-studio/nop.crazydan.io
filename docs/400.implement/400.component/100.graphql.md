...
并且只需要编写少量的适配代码便可以支持除 GraphQL 以外的 REST、gRPC 等形式的 Web 调用，
因此，其在自动化测试、非 Web 应用开发以及与其他框架集成等方面，也具有更加灵活的优势。
...
- [GraphQL Field](https://spec.graphql.org/October2021/#sec-Language.Fields):
  GraphQL 字段，用于对 GraphQL Operation 执行结果做选择性取值，仅指定字段的值才会返回给客户端。GraphQL
  Field 是嵌套结构，可嵌套定义 GraphQL SelectionSet，对于包含结构的字段值，则需要继续对其做字段取值；
- [GraphQL Selection](https://spec.graphql.org/October2021/#Selection):
  对应唯一的 GraphQL Field，因此，也可称之为 GraphQL Field Selection；
...
- [GraphQL Argument](https://spec.graphql.org/October2021/#Argument):
  GraphQL 字段参数，用于控制 GraphQL Field 的取值结果，比如，指定查询的过滤条件等。
...
根据 GraphQL 的组成结构，可以定义一个做 `query` 操作的 GraphQL Document，例如:
...
可以看到，GraphQL SelectionSet 会自上而下形成一棵树，本文称之为
**GraphQL 字段树**。在 GraphQL 字段树中的各级子字段树均表示对其父字段的取值结果做字段选择。

而在 GraphQL 字段上指定的参数，例如 `UserEntity__findPage(query: $query)`
中的 `query` 和 `roleList(limit: 10)` 中的 `limit`，则可以控制该字段的取值结果，
比如，按照 `query` 中的过滤条件做数据查询。因此，不能简单地将 GraphQL
字段当作是<u>父字段取值结果对象</u>的属性名，
而应该将其视为一个**字段取值函数**：对于简单的字段取值，就是调用父字段取值结果对象的
getter 方法获取与字段同名的属性的值，而对于复杂点的字段取值，
则需要根据字段参数获取与父字段取值结果相关的数据。
...
因此，对 GraphQL Operation 的执行过程便是沿着字段树，
自上而下调用字段取值函数的过程，该过程会持续进行，直到遇到的字段取值结果为
[标量类型](https://graphql.org/learn/schema/#scalar-types)（Scalar
Types，即整型、字符、布尔等非结构化数据）时终止。
其最终将返回与 GraphQL 字段树结构相一致的数据，例如：
...
**字段取值函数**增加了字段取值的灵活性和可扩展性，可以实现比<u>对象属性取值</u>更为复杂的字段取值需求，
比如，调用外部接口获取字段数据。

对于上例中的 GraphQL Document，则可以将其取值过程描述为以下几个步骤：
...
- 继续从 `PageBean<UserEntity>` 中取字段 `total` 和 `items` 对应的同名属性的值。
  由于 `total` 属性为标量类型，故而，在取得其值后便会终止该方向上的取值过程，而
...
- `UserEntity#roleList` 字段带有参数，因此，其实际绑定的是一个复杂的取值函数，
  其将根据参数 `limit` 查询出与当前 `UserEntity` 关联的前 `limit` 条 `RoleEntity`
  数据，并返回分页查询结果 `PageBean<RoleEntity>`；
- 继续对 `PageBean<RoleEntity>` 做字段取值，直到遇到的字段值均为标量类型为止；
- 完成字段取值，组装并返回 GraphQL Document 的取值结果；

从上述取值过程的描述中，可以发现，在 GraphQL 字段取值的过程中，
只有对子层级字段的取值是作用在其直接父级字段的取值结果上的，
而对同一层级的字段取值，则是互不影响、互不相关的。

同样，在 GraphQL Operation 中的第一层级的字段之间也没有相关性，
因此，可以在 GraphQL Operation 的第一层级中同时定义多个字段，
并且，这些字段的取值结果也不从属于某个具体的对象，
故而，本文将 GraphQL Operation 中的第一层级的字段称为 **GraphQL 根字段**。

也就是，客户端可以根据需求在 GraphQL Document 中同时指定多个根字段，例如：
...
在对 [GraphQL](#about-graphql) 有一个基本的认识后，便可以继续深入了解
