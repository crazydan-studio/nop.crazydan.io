...
那么，`@BizLoader` 加载函数的 `@ContextSource` 参数所传的 `List<UserEntity>`
会同时包含 `UserEntity__findPage` 和 `UserEntity__get` 所得到的 `UserEntity`
对象。因此，在批量查询时，需以 `UserEntity` 实例为 Map Key，以其 `RoleEntity` 列表为 Map Value，
...
- `BizObjectBuilder#buildBizLoaders`
  - `ReflectionBizModelBuilder#build`
    - `ReflectionBizModelBuilder#buildFetcher`
