...
虽然 NopGraphQL 引擎是 [GraphQL 协议](#proto-graphql)的解析和执行器，
...
不过，为了更清晰地分析 NopGraphQL 引擎的设计与实现，本文后续内容将主要围绕对
GraphQL 协议的解析和执行过程而展开。
...
NopGraphQL 引擎的整体架构如下：

```plantuml
actor "客户端" as client
control "校验器" as validator
component "GraphQLEngine" as engine
component "IBizModel" as biz
control "转换器" as transformer
component "IObjMeta" as meta

client -down-> validator: 请求 Body：\l{\lquery: 'query Xxx__findPage(...) { ... }',\lvariables: { ... }\l}
validator -down-> engine: GraphQLDocument

engine -up-> transformer: GraphQLResponseBean
transformer -up-> client: 响应 Body：\l{ data: {\lXxx__findPage: { ... }\l} }

biz -up-> engine: 返回业务对象
engine -down-> biz: 调用业务处理函数

meta 0)-right- validator
meta 0)-left- transformer
```
...
相较于 REST 等一般协议，GraphQL
协议为数据查询提供了[字段选择](https://graphql.org/learn/queries/#fields)的能力，
方便客户端按需定制返回内容，从而避免无关数据的传输，提升应用效率。

> NopGraphQL 引擎也支持 REST 接口，并为其也提供了等价的字段选择能力。

引擎自动完成 DTO 适配，极大提升数据模型的可组合性。
