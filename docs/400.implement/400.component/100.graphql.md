...
### 聚合根对象 {#aggregate-root}

<!-- TODO 整合至正文中
- thisObj 代表行为聚合对象
- BizObj 代表聚合根对象，包含行为聚合和数据聚合
-->

BizAction 的主要作用是提供一些在服务端可以调用但是又不暴露给外部客户端的函数。`mutation`、`query`
和 `action` 都可以通过 `thisObj.invoke(actionName, argsMap)` 这种方式调用。

`thisObj` 是 `BizModel + BizModel + XBiz` 这种**多重堆叠**产生的最终的聚合对象。DDD
中的聚合根对象在 Nop 平台被分解为**行为聚合**和**数据聚合**。行为聚合就是 `thisObj`
聚合多个切片信息，而数据聚合就是传统的领域模型对象**通过关联属性实现聚合**。

在外部微服务的调用者眼中，一个 `BizObj` 就是一个聚合根对象。通过 GraphQL 的
Selection 机制，我们可以选择从聚合根对象上截取部分信息。
...
- NopGraphQL 可以实现对象级别的动态模型加载，而一般的 GraphQL 是整体定义
  ...
  根据名称进行加载的时候会实现动态模型更新，这一机制是 Nop
  平台系统化的一种处理方案，即把动态性隐藏在加载器背后，整体系统逻辑实现线性化；
...
- `CrudBizModel = NopGraphQL + NopORM`，直接将 ORM 的能力作为微服务暴露。而且
  NopORM 底层的 driver 处理 `JdbcPersisterDriver` 之外，还可以使用 GraphQL
