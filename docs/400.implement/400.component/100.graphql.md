...
- [GraphQL SelectionSet](https://spec.graphql.org/October2021/#SelectionSet):
  GraphQL 选择字段集，以一对花括号 `{`、`}` 包裹起来的 GraphQL Selection 集合，通过逗号
...
- 控制行：`CrudBizModel#prepareFindPageQuery`
  -> `AuthHelper#appendFilter`
  -> `IDataAuthChecker#getFilter`
  -> `DefaultDataAuthChecker#getFilter`
- 控制列：XMeta 中的 [`<auth />`](../../manual/xmeta#attr_props_prop_auth) 配置

## Validator 模型 {#biz-validator}
