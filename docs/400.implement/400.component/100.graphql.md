...
的，也不依赖 Spring MVC、Hibernate 等具体的技术框架，
...
- **标量类型**：Scalar Types，即整型、字符串、布尔等非结构化数据类型；
- **对象类型**：Object Types，即存在结构的数据类型；
...
- [GraphQL Field](https://spec.graphql.org/October2021/#sec-Language.Fields):
  GraphQL 字段，用于对 GraphQL 的查询或变更结果做字段取值，仅指定字段的值才会返回给客户端。GraphQL
  Field 是嵌套定义结构，可嵌套定义 GraphQL SelectionSet。对于对象类型的字段，必须继续对其做字段取值；
...
**GraphQL 字段树**。在 GraphQL 字段树中的各级子字段树均表示对其父字段的取值结果做**字段取值**，
也就是，从父字段的取值结果中**仅**获取指定的子字段的值。

而在 GraphQL 字段上指定的参数，例如 `UserEntity__findPage(query: $query)`
中的 `query` 和 `roleList(limit: 10)` 中的 `limit`，则可以用于控制该字段的取值结果，
比如，根据 `query` 参数中的过滤条件获取相应的字段值。因此，不能简单地将 GraphQL
字段当作是<u>父字段取值结果对象</u>的属性名，
而应该将其视为一个**字段取值函数**：对于简单的字段取值，就是调用父字段取值结果对象的
getter 方法获取与字段同名的属性的值，而对于复杂的字段取值，
...
因此，对 GraphQL Document 的取值过程便是沿着字段树，
自上而下调用字段取值函数的过程，该过程会持续进行，
直到遇到的字段取值结果为[标量类型](https://graphql.org/learn/schema/#scalar-types)时终止。
最终得到的 GraphQL Document 取值结果与其字段树的结构是一致的，
在 GraphQL Document 中未指定字段的值不会出现在其取值结果中，例如：
...
> 注意，作为取值结果的属性名，GraphQL 字段的别名（`Alias`）优先于其字段名（`Name`）。
> 而对于元素为对象类型的列表，则需要遍历列表，并依次对列表元素做字段取值。
...
- 继续从 `PageBean<UserEntity>` 中取字段 `total` 和 `items` 对应的同名属性的值。
  ...
  `items` 属性的类型为 `List<UserEntity>`，其为对象类型列表，
  因此，继续对其列表元素做字段取值；
...
- 完成字段取值，以字段别名或字段名作为属性名组装并返回 GraphQL Document 的取值结果；

从上述取值过程的描述中可以发现，对 GraphQL Document
的取值是从第一层级的字段（即上例中的 `UserEntity__findPage`）取值结果开始的，
并且，其取值结果也不从属于某个具体的对象，故而，本文将 GraphQL Document
中第一层级的字段称为 **GraphQL 根字段**。

而在同一个 GraphQL Document 中则可同时指定多个根字段，并且还可为根字段指定别名，例如：
...
类型，并且在 `query` 操作中不能做数据变更（当然，也没有措施严格执行该约定），而在 `mutation`
...
state "GraphQLSelectionResolver#resolveOperation(doc, op)" as call_resolver_resolveOperation {
  ...
  state "递归初始化字段：GraphQLSelectionResolver~#resolveFieldSelection(objName, fieldDef, field)" as call_resolver_resolveFieldSelection {
    ...
    state "GraphQLEngine#getTypeDefinition(objName)" as call_engine_getTypeDefinition: 得到业务对象 objName 的 GraphQL 对象类型\n<b>objDef:GraphQLObjectDefinition</b>（根据 IObjMeta 生成）
...
而通过 `IBizObject#objMeta` 和 `IBizObject#bizModel` 便可以分别构造出 GraphQL 对象类型
...
`fieldDef.getType().getNamedTypeName()`）得到业务对象的 GraphQL 对象类型
