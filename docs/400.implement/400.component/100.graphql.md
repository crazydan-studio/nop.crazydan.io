...
component "GraphQLEngine" as engine {
  ...
  control "聚合 DataFetcher 的字段值" as merge
...
client -right-> parser: 提交 GraphQL Document 文本
parser -right-> doc: 校验 GraphQL Document\n并绑定 GraphQLFieldDefinition
```
...
NopGraphQL 引擎对 GraphQL 的驱动过程由客户端向 Web 端点 `/graphql` 发起接口调用而触发。
...
假设客户端需要向 NopGraphQL 提交的 GraphQL Document 如下：
...
则客户端在发起 `/graphql` 调用时，需以 JSON 对象形式同时回传
GraphQL Document 文本和变量 `$query`、`$id`：

```json
{
  // GraphQL Document 文本内容
...
接着，NopGraphQL 将按照以下过程解析 GraphQL Document 文本，以得到
`GraphQLDocument` 对象：
...
在解析得到 `GraphQLDocument` 对象后，在 `GraphQLEngine` 中将继续对其进行初始化，
以进一步校验其结构的有效性，并为 GraphQL Field 补充字段定义 `GraphQLFieldDefinition`：
