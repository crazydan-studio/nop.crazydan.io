...
`IBizModel`（**业务模型**）和 `IObjMeta`（**业务对象元数据**）组成。

同时 `IBizObject` 还包含 GraphQL 的结构信息：
...
不会对其做深度遍历，仅在 GraphQL 字段取值时，再根据其名字得到 `GraphQLObjectDefinition` 进而做深度遍历取值。
...
NopGraphQL 要求所有属性都必须在 xmeta 文件中声明，
