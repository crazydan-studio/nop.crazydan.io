  ...
  * Operation
    ...
    * VariableDefinitions <<opt>>
      ...
      * VariableDefinition
        * Variable
          * $ <<literal>>
          * [[https://spec.graphql.org/October2021/#Name Name]] <<literal>>
        * : <<literal>>
        * [[https://spec.graphql.org/October2021/#Type Type]] <<literal>>
        * DefaultValue <<opt>>
          * <U+003D> <<literal>>
          * [[https://spec.graphql.org/October2021/#Value Value]] <<literal>>
      * , <<literal>>
      * VariableDefinition
      * , <<literal>>
      * ...
...
的变量 `$query`，该变量需由客户端随 GraphQL 端点调用一起传给服务端，再由服务端将其转换为
`QueryBeanInput` 对象后，传递到 GraphQL Field 的参数列表中。
...
向下钻取将会按照 GraphQL Document 的结构一直进行下去，直到遇到字段值为字面量（非结构化数据）时才终止。

因此，在前例中，可以这样理解 GraphQL Document 的数据钻取过程：

- 调用 `UserEntity__findPage` 字段对应的 `IDataFetcher` 以获取分页结果 `PageBean`，
  而其参数 `query` 将被作为相关接口的调用参数以参与过滤查询；
- 得到上一层级的结果 `PageBean` 后，继续钻取其属性 `total` 和 `items`，由于 `total` 为字面量，
  故而，对其的钻取结束，而 `items` 是一个对象列表（假设列表元素为 `UserEntity` 类型），
  故而，需继续向下钻取；
- 遍历 `UserEntity` 列表，依次钻取每个对象的 `id`、`name` 和 `roleList` 属性，
  这里只有 `roleList` 是一个对象列表（假设列表元素为 `RoleEntity` 类型），
  因此，继续向下钻取 `roleList`；
- `roleList` 对应的是一个关联列表，并且，在 GraphQL Document 中设置了参数 `limit` 为 `10`，
  表示仅取前 `10` 条数据，因此，该字段对应的 `IDataFetcher` 将以上一级的 `UserEntity`
  对象作为过滤条件，以查询与其绑定的 `RoleEntity`；
- 在获取到 `roleList` 的结果后，继续向下钻取每个列表元素，直到没有可钻取的对象；
- 由于为 `roleList` 设置了别名 `roles`，因此，该列表的最终钻取结果将赋值给 `roles` 属性；

注意，`IDataFetcher` 是一个通用的 GraphQL Field 钻取接口，除了可以做列表查询，
还可以调用上一级对象的 getter 接口获取与 GraphQL Field 同名的属性值，
从而实现对字面量属性的钻取，比如，钻取 `PageBean` 的 `total` 和 `items` 属性，
以及 `UserEntity`/`RoleEntity` 的 `id`、`name` 属性。

不过，`IDataFetcher` 与 GraphQL Field 的绑定不是随意确定的，而是根据 GraphQL Schema
自动推断的，而在 NopGraphQL 中，GraphQL Schema 则是由 [XMeta](../../manual/xmeta)
和 [BizModel](../../manual/xbiz) 动态生成的。

从上述对 GraphQL Field 钻取过程的描述可以感受到 GraphQL 在定制数据输出方面的灵活性，
客户端完全可以按照自身需求指定返回包括子查询在内的复杂数据结构，甚至可以在一个 GraphQL Operation
中同时指定多个查询：
...
而这种灵活性并不会造成开发的负担，大部分 GraphQL Field 都可以进行自动推理以绑定上相对应的
`IDataFetcher`，即使需要应对复杂的数据钻取需求，在 NopGraphQL 中也可以通过
XMeta 和 BizModel 动态实现，并且，其原生支持差量机制，从而让 GraphQL 具备了无限可能。
