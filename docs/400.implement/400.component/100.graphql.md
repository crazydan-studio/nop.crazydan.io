...
query (
  ...
  UserEntity__findPage(
    ...
    items {
      id, name
      # 定义指定了 Alias 的 Field，
      # 并使用 roles 作为返回结果的属性名
      roles: roleList(limit: 10) {
        items {
          id
          # 使用 roleName 作为返回结果的属性名
          roleName: name
        }
      }
    }
...
可以看到，GraphQL SelectionSet 会自上而下形成一棵**字段树**。
在 GraphQL 字段树中的各层级的**子字段树**，则表示对其父字段的<u>字段取值结果</u>做**字段选择**，
也就是，在 GraphQL 的**返回结果**中，均只包含子字段对应的属性值，
其父字段取值结果对象上的其他属性均会被忽略。比如上例 GraphQL Document 的返回结果便为如下形式：
...
在 GraphQL 返回结果中，默认会以对应的 GraphQL 字段名作为其属性名，而若是为字段指定了别名，
如 `roles: roleList` 和 `roleName: name`，则会优先以字段别名 `roles` 和 `roleName`
作为返回结果的属性名。并且，在字段取值结果是列表类型，且列表元素为对象类型时，GraphQL 会遍历列表，
并对列表元素做字段选择，再将字段选择后的结果按遍历顺序组装到 GraphQL 返回结果中。

而处于第一层级的字段，如 `UserEntity__findPage`，是 GraphQL 字段树进行字段选择的根节点，
并且其返回结果也不从属于某个具体的对象，因此，本文称其为 **GraphQL 根字段**。

通常情况下，可以将 GraphQL 字段名视为父字段取值结果对象的属性名，而对父字段做字段选择，
就是从其取值结果对象上取与子字段同名的属性值。但是，GraphQL 支持在字段上指定参数，例如
`UserEntity__findPage(query: $query)` 中的 `query` 和 `roleList(limit: 10)`
中的 `limit`，其作用是控制该字段的取值结果，比如，根据 `query` 参数中的过滤条件查询得到字段的值。
...
而应该将其视为一个**字段取值函数**：对于简单的字段取值，就是直接调用父字段取值结果对象的
getter 方法便可得到子字段的值，而对于复杂的字段取值，
则需要根据字段参数才能获取到与其父字段取值结果相关的数据。
并且，通过字段取值函数，可以屏蔽不同字段的取值差异，统一字段取值机制，让
GraphQL 的字段取值更加灵活方便，也能够支持如批量加载、拉取远程数据等复杂的取值需求。

对于上例中的 GraphQL Document，则可以将其字段选择过程描述为以下几个步骤：
...
- 完成字段选择，以字段别名或字段名作为属性名组装并得到 GraphQL 返回结果；

当然，以上分析的只是对简单的 GraphQL Document 做字段选择的情况，在复杂的场景下，还可以在同一个
GraphQL Document 中同时指定多个根字段，并且为根字段指定别名，例如：
