...
顾名思义，NopGraphQL 引擎本质是 [GraphQL DSL](#about-graphql) 的解析和执行器，
其会按照给定的 [GraphQL Document](https://spec.graphql.org/October2021/#sec-Document)
执行操作并返回指定的字段数据。

因此，在深入了解 NopGraphQL 的设计与实现之前，首先需要搞清楚以下问题：
...
Nop 平台之所以采用 GraphQL 作为服务端与客户端之间的接口交互协议，主要是因为 GraphQL 具有以下诸多优点：

- GraphQL 具备[字段选择](https://graphql.org/learn/queries/#fields)能力，
  可以实现**对数据的自动裁剪**，**控制数据的加载范围**：
  仅组装和传输客户端需要的字段数据，不需要的数据既不会在后端做无用加载，也不会强行返回给客户端；
- GraphQL 支持对 DTO（Data Transfer Object，即数据传输对象）模型的自由组合，
  客户端可以按需获取不同结构的数据，而服务端无需为各类需求做专门的处理和开发；
- GraphQL 中需明确指定具体的访问字段，因此，可以更加明确地对各个字段做数据访问权限控制，
  从而提升系统安全性；
- GraphQL 中所指定的各个字段均有独立的加载器，可以更加灵活地处理各种字段的返回数据，
  对其做输入/输出转换、复杂的关联查询等，且不会增加服务端的开发复杂度；
- GraphQL 具有明确的副作用语义 `mutation`，因此，可以有针对性地进行事务管理等操作，
  避免不必要的性能损耗，提升应用性能；

> 点击[这里](https://chatglm.cn/share/Fj0iT)查看智谱清言 AI 针对以上问题的回答。

- [GraphQL](https://graphql.org/learn/)

```graphql
# TODO 各类样例
query UserEntity__findPage($query: QueryBeanInput) {
