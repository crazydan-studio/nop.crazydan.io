...
[XMeta](../../manual/xmeta) 和 [XBiz](../../manual/xbiz) 动态实现，
...
在找到第一级字段的字段定义后（记录在 `IBizObject` 中），便可以该字段定义为根节点，
沿着客户端提供的 GraphQL Document 的结构做广度+深度遍历，并在遍历过程中获取到当前字段的字段定义，
若字段没有字段定义，则其结构无效，中止遍历并抛出校验异常，否则，将字段与其字段定义绑定在一起，
以便于后续进行**字段取值**操作。

完成对 `GraphQLDocument` 的初始化后，接下来，便需要沿着其结构获取字段值了。

### 取 GraphQLDocument 字段值 {#impl-fetch-graphql-field}

在完成对 `GraphQLDocument` 的[初始化](#impl-init-graphql-doc)后，其各级字段便与对应的字段定义
`GraphQLFieldDefinition` 进行了绑定，而在 `GraphQLFieldDefinition` 上会提供 `fetcher` 函数，
用于对字段取值，因此，对 `GraphQLDocument` 取字段值的过程，就是沿着其 GraphQL Field Selection
构成的 GraphQL 字段树，挨个调用**字段取值函数**（`GraphQLFieldDefinition#fetcher`），
并返回与字段树具有相同结构的取值结果的过程。

仍然以以下 GraphQL Document 为例：
...
在 NopGraphQL 中，GraphQL 调用返回给客户端的是 `GraphQLResponseBean` 对象，
其中，`GraphQLResponseBean#data` 属性则作为根节点，存放的是 `GraphQLDocument` 的取值结果：
...
- `ServiceActionFetcher`: 调用业务模型（如，`UserEntityBizModel`）上的操作函数，
  操作函数的返回值即为 GraphQL 的字段值。在 GraphQL Field 上指定的参数，
  也将作为该操作函数的参数，在调用时传入；
- `BeanPropertyFetcher`: 获取在 Java Bean 上的属性值，
  其将从当前字段所属的上级字段的结果对象中钻取属性值；

在 GraphQL Operation 中的第一级字段均映射的是业务模型 [BizModel](#biz-model) 的操作函数，
其字段名字为 `{bizObjName}__{bizAction}` 形式，其中，`{bizObjName}`
为业务模型名（如 `UserEntity`），`{bizAction}` 为业务模型上定义的操作函数名（如 `findPage` 与 `get`）。

> 业务模型名通过注解 `@BizModel` 指定，
> 可用于 GraphQL 字段取值的操作函数名则通过注解 `@BizQuery`/`@BizMutation` 指定。

对业务模型操作函数的调用封装在 `ServiceActionFetcher` 中，在执行时，
其会将 GraphQL 字段上指定的参数传入操作函数，并对返回结果继续做字段取值。

例如，`UserEntity__findPage` 的返回结果为 `PageBean` 类型，并且该类型的 getter
方法也标注了注解 `@PropMeta`，可以将其视为以强类型方式定义的 [ObjMeta](#obj-meta) 结构。

> 对 `PageBean` 的结构分析逻辑见 `ReflectionGraphQLTypeFactory#buildPageBeanType`。

NopGraphQL 限定只有定义了 [ObjMeta](#obj-meta) 结构的对象才能做字段取值，而对此类对象的字段取值，
则默认采用 `BeanPropertyFetcher` 函数，其会直接调用对象上的 getter 方法（如 `UserEntity#getId()`），
从而取得与字段同名的对象属性的值。

> 默认字段取值函数的确定逻辑见 `GraphQLExecutor#hookFetch`。

除此以外，还可以在 BizModel 的函数上标注 `@BizLoader`（GraphQL 字段加载器），NopGraphQL
会将此类**GraphQL 字段加载函数**封装在字段取值函数 `EvalActionDataFetcher` 中，
在字段取值过程中遇到与 `@BizLoader` 标注的名字相同的字段时，
则最终会调用 BizModel 的字段加载函数获取字段值。

而若是在 XMeta 中为对象属性配置了
[&lt;transformOut />](../../manual/xmeta#attr_props_prop_transformOut)、[&lt;getter />](../../manual/xmeta#attr_props_prop_getter)
等，则会通过执行相应的 Xpl 函数得到字段值。

## 引擎核心模型 {#engine-core-models}
...
obj_prop_meta_inf <|-- obj_prop_meta
...
biz_obj "1" *-- "1" gpl_obj_def: 包含 >

biz_model "1" *-- "*" biz_action: 包含 >
obj_meta "1" *-- "*" obj_prop_meta: 包含 >
gpl_obj_def "1" *-- "*" gpl_field_def: 包含 >
gpl_field_def "1" *-- "1" fetcher_inf: 包含 >

gpl_obj_def - obj_meta: 引用 >
gpl_field_def - obj_prop_meta: 引用 >
```

`IBizObject` 代表**业务对象**，其主要由
`IBizModel`（**业务模型**）和 `IObjMeta`（**业务对象实体元数据**）组成。
`IBizModel` 由业务操作函数（由 `IBizActionModel` 表达）和 GraphQL 字段加载函数（由
`BizLoaderModel` 表达）组成，而 `IObjMeta` 则描述的是**业务对象实体**的结构，
包含实体的全部属性定义（由 `IObjPropMeta` 表达）。

需要注意的是，由于 NopGraphQL 要求 GraphQL Document 中的第一级字段取值必须映射到业务模型的
`query` 或 `mutation` 操作函数（分别在类方法上标注 `@BizQuery` 或 `@BizMutation`）上，
在操作函数均不返回业务对象实体的情况下，是不需要存在 `IObjMeta` 的，
因此，只有 `IBizModel` 是必须的，而 `IObjMeta` 是可选的。

此外，`IBizObject` 还包含如下两类 GraphQL 的结构信息，用于对 GraphQL Document
进行有效性校验和字段取值：

- `objectDefinition`: 类型为 `GraphQLObjectDefinition`，
  表示的是业务对象的 GraphQL 字段定义，其根据 `IObjMeta` 上的属性（`IObjPropMeta`）列表生成；
- `operations`: 类型为 `Map<String, GraphQLFieldDefinition>`，其由 BizModel
  中的 `query` 和 `mutation` 操作函数生成；
...
- **内部函数**：标注了 `@BizAction` 的函数，在启用了 maker checker 机制后，用于检查操作是否合法等。
  详细处理逻辑见 `ObjectDefinitionExtProcessor#initMakerChecker`、`MakerCheckerTryServiceAction#sendForCheck`
  与 `GraphQLExecutor#invokeOperationOrTry`；
...
`DataLoader#load` 的作用就是将参数收集起来，再等待 `DataLoader#dispatch`
被调用，`DataLoader#dispatch` 会将收集到的结果作为参数传给
`BatchLoader#load`，从而真正执行 `@BizLoader` 加载函数。

对 `DataLoader#dispatch` 的调用最终由 `GraphQLExecutionContext#dispatchAll`
触发，其会调用所有已注册的 `DataLoader` 的 `dispatch` 方法，并等待所有
`DataLoader` 返回加载到的数据。

最终，便实现了在 GraphQL Document 上下文范围内的批量加载支持，
其可以对任意深度和广度的参数收集，并以此批量加载其相关数据，
再依次返回给对应位置的 GraphQL 字段。

由于 `DataLoader` 采用的是 Promise 异步调用，在正常的 GraphQL 字段取值时，
`DataLoader#load` 会被调用，但其返回的 `CompletableFuture` 需要由
`DataLoader#dispatch` 触发执行，其不会在字段取值时继续深度取值，
因此，其处于中断状态，只有在 `GraphQLExecutionContext#dispatchAll`
触发后，才继续后续的 GraphQL 字段取值。故而，可以保证字段取值的准确性，不会发生丢失或赋值混乱的情况。
...
| 类型 | 注解 | 说明 |
| ---- | --- | --- |
| `IServiceContext` 及其子类 | 无 | GraphQL 请求操作的上下文 |
| `ParsedGraphQLRequest` | `@ContextRoot` | 解析后的 GraphQL 请求，包含 `GraphQLDocument` 以及回传的变量列表。可通过 `IServiceContext#getRequest()` 获取 |
| `IContext` | 无 | 操作函数执行上下文。可通过 `IServiceContext#getContext()` 获取 |
| `IEvalScope` 及其子类 | 无 | Xpl 脚本执行的作用域。可通过 `IServiceContext#getEvalScope()` 获取 |
| `IUserContext` 及其子类 | 无 | 用户上下文。可通过 `IServiceContext#getUserContext()` 获取 |
| `ICache` 及其子类 | 无 | 用于缓存字典、关联对象等数据。可通过 `IServiceContext#getCache()` 获取 |
| `FieldSelectionBean` 及其子类 | 无 | 第一级 GraphQL Field Selection 的结构，含参数、子字段等信息 |
| `ApiRequest` | 无 | 以第一级 GraphQL 字段参数作为 `ApiRequest#data`（类型结构需与 GraphQL 字段参数一致）构造出的 `ApiRequest` 类型数据，其中，`ApiRequest#selection` 与操作函数的 `FieldSelectionBean` 类型参数相同，`ApiMessage#headers` 来自于 `IServiceContext#getRequestHeaders()` |
...
| 类型 | 注解 | 说明 |
| ---- | --- | --- |
| 不定 | `@Name` | 。除注解标注和内部参数以外，参数都必须命名 |
| 不定 | `@ContextRoot` |  |


`IServiceContext` IDataFetchingEnvironment::getServiceContext
`IEvalScope` IDataFetchingEnvironment::getEvalScope
`IUserContext` env -> env.getServiceContext().getUserContext()
`IContext` IDataFetchingEnvironment::getContext
`ICache` IDataFetchingEnvironment::getCache
`FieldSelectionBean` IDataFetchingEnvironment::getSelectionBean

`@ContextSource` IDataFetchingEnvironment::getSource 至多只能有一个
`@RequestBean` getArgsAsBean(arg.getType()) 一般只有一个

`IDataFetchingEnvironment` getEnv()
`ApiRequest` getArgsAsRequest(arg.getType().getTypeParameters().get(0))
