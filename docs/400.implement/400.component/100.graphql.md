...
向下钻取将会按照 GraphQL Document 的结构一直进行下去，直到遇到字段值为标量类型（非结构化数据）时才终止。
...
- 得到上一层级的结果 `PageBean` 后，继续钻取其属性 `total` 和 `items`，由于 `total` 为标量类型，
...
从而实现对标量类型属性的钻取，比如，钻取 `PageBean` 的 `total` 和 `items` 属性，
