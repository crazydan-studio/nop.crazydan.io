...
Nop 平台采用 GraphQL 的原因为：

- [字段选择](https://graphql.org/learn/queries/#fields)能力；
- 可自动完成 DTO 适配，极大提升数据模型的可组合性；

```plantuml
actor "客户端" as client
control "适配器" as adaptor
file "GraphQL\nDocument" as doc

component "GraphQLEngine" as engine {
  control "执行字段 DataFetcher" as exec
  control "合并字段数据" as merge

  doc -down-> exec: 提供字段 DataFetcher
  exec -left-> merge: 提供执行 DataFetcher 得到的字段数据
  merge -up-> client: 返回操作结果
}

client -right-> adaptor: GraphQL\nREST\ngRPC\n...
adaptor -right-> doc: 生成 GraphQL Document
```
