...
rectangle "NopGraphQL" as graphql {
  portin "GraphQL" as api_p1
  portin "REST" as api_p2
  portin "gRPC" as api_p3
  portin "..." as api_pn
  ...
  api_graphql_adaptor -right-> engine: GraphQLRequestBean
  engine -left-> api_graphql_adaptor: GraphQLResponseBean

  api_rpc_adaptor -left-> engine: ApiRequest
  engine -right-> api_rpc_adaptor: ApiResponse
...
> - 接口适配器包括 `io.nop.graphql.core.web.GraphQLWebService`
