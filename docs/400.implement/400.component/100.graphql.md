...
并且下层子树的字段取值是依赖于上层字段的取值结果的，一般会在父字段的结果对象上做属性取值。
...
为此，NopGraphQL 对 `PageBean` 做了专门的处理，以为其单独构造 `GraphQLObjectDefinition`，
从而确保字段取值的一致性。

> 对 `PageBean` 的处理逻辑祥见 `ReflectionGraphQLTypeFactory#buildPageBeanType` 的实现。

在完成对 `GraphQLDocument` 的取值后，NopGraphQL 最终会以其取值结果构造
`GraphQLResponseBean` 作为服务调用的结果返回给客户端。而在
