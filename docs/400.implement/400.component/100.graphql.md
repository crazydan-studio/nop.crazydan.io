...
这在涉及对多个 `UserEntity` 的实例做字段选择时，会存在较大的性能问题。
...
其在调用 `invokeOperations(DataFetchingEnvironment)` 方法得到 $P_0$ 后，
...
再将得到的结果按顺序传给各个 Promise 对象，从而继续后续的字段取值操作。
...
> 操作函数的参数解析逻辑见 `ReflectionBizModelBuilder#buildFetcher`。
