...
getter 方法作为取值函数（也即 `BeanPropertyFetcher`）。
...
在本文的[《核心模型》](#engine-core-models)章节中已经提到 NopGraphQL 要求
...
state "GraphQLSelectionResolver#resolveOperation(doc, op)" as call_resolver_resolveOperation {
  ...
  state "递归初始化字段：GraphQLSelectionResolver~#resolveFieldSelection(objName, fieldDef, field)" as call_resolver_resolveFieldSelection {
    ...
    state "fieldDef.getType().getNamedTypeName()" as call_fieldDef_getType: 从字段定义 fieldDef 中得到当前字段值对应的\n业务对象名字 **objName**
    state "GraphQLEngine#getTypeDefinition(objName)" as call_engine_getTypeDefinition: 得到业务对象 objName 的 GraphQL Schema 对象\n<b>objDef:GraphQLObjectDefinition</b>（根据 IObjMeta 生成）
...
> `BizObjectManager#getBizObject(bizObjName)` 只是根据业务对象的名字 `bizObjName`
> 从缓存中获取其 `IBizObject` 的实例而已。对业务对象构造过程的分析请阅读章节
> [《BizObject》](#biz-object)。

由于 NopGraphQL 对 `GraphQLDocument` 的校验，主要是检查字段是否存在、字段树深度是否超出限制等，
因此，本章节不会对其校验逻辑进行展开分析，详细的处理逻辑可以按照上图中的代码调用链，
查看具体的代码实现进行了解。
...
也就是，首先需要通过根字段的名字得到其定义，然后再按照普通字段的方式对根字段及其子字段进行递归初始化。

根字段的定义直接根据对应的业务对象 `bizObj` 调用其 `getOperationDefinition` 方法即可。

而要获得子字段的定义，则需要先通过当前字段 `field` 的字段定义 `fieldDef` 得到字段值所对应的业务对象名字
`objName`（对应的代码调用为 `fieldDef.getType().getNamedTypeName()`），再调用
`GraphQLEngine#getTypeDefinition(objName)` 得到子字段所属的业务对象的
GraphQL Schema（由 `GraphQLObjectDefinition` 模型描述）对象 `objDef`，
最后，才能根据子字段名字从 `objDef` 中得到（对应调用 `objDef#getField`）其字段定义。

在找到字段的字段定义后，便可以将字段定义与字段绑定，以便于后续做字段取值，
并可以继续进行字段参数的有效性校验等初始化工作（根字段与其子字段的初始化过程是相同的）。
若不能为字段找到其定义，则其必然不是有效的 GraphQL Document，直接抛出异常即可。

在沿着 GraphQL Document 字段树完成对 `GraphQLDocument` 的字段初始化和有效性校验后，
接下来，便需要对 `GraphQLDocument` 进行字段取值了。

#### 取 GraphQLDocument 字段值 {#engine-fetch-graphql-field}

在完成对 `GraphQLDocument` 的[初始化](#engine-init-graphql-doc)后，其各级字段便与对应的字段定义
