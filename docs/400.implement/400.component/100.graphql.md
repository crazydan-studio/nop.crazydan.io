...
rectangle "NopGraphQL" as graphql {
  ...
  portin " " as api_p3
  portin " " as api_pn
  control "接口适配器" as api_adaptor

  component "Engine" as engine
  file "XMeta" as xmeta
  file "BizModel" as biz_model

  api_p1 -down-> api_adaptor
  api_p2 -down-> api_adaptor
  api_p3 -down-> api_adaptor
  api_pn -down-> api_adaptor
  api_adaptor -down-> engine

  xmeta -right-> engine
  biz_model -left-> engine
}

client_1 -down-> api_p1: GraphQL
client_2 -down-> api_p2: REST
client_3 -down-> api_p3: gRPC
