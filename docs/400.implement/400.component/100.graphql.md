...
rectangle "NopGraphQL" as graphql {
  ...
  component "GraphQLEngine" as engine {
    port " " as api_call_graphql
    port " " as api_call_rpc
    port " " as api_return_graphql
    port " " as api_return_rpc

    component "BizObjectManager" as biz_obj_man
    ...
    executor -> api_return_graphql: GraphQLResponseBean
    executor -> api_return_rpc: ApiResponse

    query -down-> biz_obj_man
    xmeta -up-> biz_obj_man
    xbiz -up-> biz_obj_man

    biz_obj_man -right-> executor: GraphQLOperation
  }
