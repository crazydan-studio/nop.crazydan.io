...
的，没有依赖于 Web Servlet、Hibernate 等具体的技术框架，
...
- **GraphQL 字段树**：在 GraphQL Document 中，由 GraphQL Field Selection 组成的结构是树形的，
  因此，本文将 GraphQL Field Selection 组成的树形结构称为**字段树**；
...
顾名思义，NopGraphQL 引擎实际上是 [GraphQL](https://graphql.org/learn/) 协议的服务端实现，
其会根据客户端给定的
...
`GraphQLFieldSelection` 便绑定了相应的字段定义 `GraphQLFieldDefinition`，
再次沿着 `GraphQLDocument` 的字段树调用取值函数 `GraphQLFieldDefinition#fetcher`，
便可以得到与 GraphQL Document 相对应的返回数据。
...
在沿着 GraphQL 字段树取值时，
子字段树的字段取值是依赖于其父字段的取值结果的（通常是在父字段的结果数据上做属性取值），
只有从父字段的字段取值结果中，取出子字段对应的属性值（也就是对父字段的取值结果做**字段选择**），
并将这些属性值组装后的数据才是最终返回给客户端的数据。
...
对 `GraphQLDocument` 的取值逻辑可用如下代码表示：
...
> 对于标注了 `@GraphQLObject` 的 Java Bean 也能够为其自动构造
> `GraphQLObjectDefinition`（构造逻辑见 `ReflectionGraphQLTypeFactory#newObjectDefinition`）。
...
业务对象 `IBizObject` 的组成已经在[《核心模型》](#engine-core-models)章节做了说明，
本章节主要分析其构造过程。
...
然后，通过 `IObjMeta` 和 `IBizModel` 便可以分别构造 GraphQL 对象类型
`IBizObject#objectDefinition` 以及 GraphQL 操作字段列表 `IBizObject#operations`。

以下为构造 `IBizObject` 的详细处理逻辑：
...
activate biz_obj_man
  ...
  activate biz_obj_builder
    ...
    activate biz_obj_builder
      ...
      biz_obj_builder -> obj_meta_gql_def: 传参 IObjMeta
...
函数的作用是为对应的 GraphQL 字段提供自定义的数据加载支持，
因此，需要将其构造为 GraphQL 字段定义，并合并到 `GraphQLObjectDefinition` 中。
...
> 或者是在 [XBiz](#xbiz) 模型中的 `<query />` 和 `<mutation />` 标签。

如果在 `IObjMeta` 中存在关联其他业务对象的属性 `IObjPropMeta`，
则仅需以该业务对象的名字构造 `GraphQLNamedType`（调用 `GraphQLTypeHelper#namedType`），
并赋值给对应字段定义 `GraphQLFieldDefinition` 的 `type` 属性即可，
不会再对其结构做深度遍历。在进行 GraphQL 字段取值时，会先根据字段关联的业务对象名字得到其
`GraphQLObjectDefinition`，再根据其结构做深度取值。

> 可以阅读 `ObjMetaToGraphQLDefinition#toGraphQLType` 的实现了解其处理逻辑。
...
>   则对于返回该实体类型数据的函数，均不需要再添加注解 `@GraphQLReturn`；
> - 若是在泛型的 BizModel 上定义的公共函数，如 Nop 内置的 `CrudBizModel#findPage` 等，
>   则必须配置注解 `@GraphQLReturn`；
> - 如果操作函数的返回值类型为 `CompletionStage`，则表示该方法是异步执行的；

BizModel 可以不继承任何基类，也不需要实现任何接口，更不需要对返回结果做 JSON
序列化（统一采用 GraphQL 字段取值机制）。仅需要在其 Java 类上标注 `@BizModel`
以指定其业务对象名，再在其所在工程模块的 `*.beans.xml` 文件中注册 Java Bean
即可，NopGraphQL 将会自动完成扫描、加载和合并工作。
...
> 同名操作函数可通过注解 `@jakarta.annotation.Priority` 指定优先级，
