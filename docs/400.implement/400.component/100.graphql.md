...
因此，对 `GraphQLDocument` 的字段选择逻辑可以简化为以下代码：
...
> 对异常情况的处理可以详见 `GraphQLEngine#toGraphQLResponse` 的处理逻辑。
