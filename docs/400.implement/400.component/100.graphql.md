...
```plantuml
actor "GraphQL\nREST\ngRPC\n..." as client
control "解析 GraphQL Document" as parse_doc
control "初始化 GraphQLDocument" as init_doc
file "~*.xmeta" as xmeta
file "~*.xbiz" as xbiz
component "GraphQLDocument" as doc

component "GraphQLEngine" as engine {
  control "遍历 GraphQL Field\n并调用字段的 DataFetcher" as exec
  control "按 GraphQL Document 的结构\n聚合 DataFetcher 的字段值" as merge

  doc -down-> exec: 提供 Document Operation
  exec -left-> merge: 提供 DataFetcher 获取的字段值
  merge -up-> client: 返回 GraphQL Document 的执行结果
}

xmeta -down-> init_doc: IObjMeta
xbiz -down-> init_doc: IBizModel

client -right-> parse_doc: 提交 GraphQL Document
parse_doc -right-> init_doc
init_doc -right-> doc
```
...
```plantuml
state "GraphQLEngine\n~#resolveSelections" as call_engine_resolveSelections

state "GraphQLSelectionResolver#resolveSelection" as call_resolver_resolveSelection: 从 doc 中得到 GraphQL Operation 对象 <b>op:GraphQLOperation</b>

[*] -> call_engine_resolveSelections: doc
...
state "GraphQLSelectionResolver#resolveOperation" as call_resolver_resolveOperation {
  state "遍历 op 中的第一级 GraphQL Field Selection" as get_field_from_op: 定义循环变量为 <b>field:GraphQLFieldSelection</b>
  state "从 field#name 中提取 <b>objName</b>" as split_objName_from_op: 直接取 field#name 中双下划线之前的部分
  ...
  state "GraphQLEngine~#getOperationDefinition" as call_engine_getOperationDefinition {
    [*] -right-> call_bizObj_getOperationDefinition: field#name
    call_bizObj_getOperationDefinition -down-> [*]: fieldDef

    state "BizObjectManager#getOperationDefinition" as call_bizObj_getOperationDefinition {
      state "按照 **bizObjName__bizAction** 形式从 field#name 中提取出\n**bizObjName** 和 **bizAction**" as split_field_name
      state "<color:red>BizObjectManager\n<color:red>~#getBizObject" as call_biz_obj_man_getBizObject: 根据 bizObjName 得到\n<b>bizObj:IBizObject</b>
      state "bizObj#getOperationDefinition" as call_biz_obj_getOperationDefinition

      [*] -right-> split_field_name: field#name
      split_field_name -down-> call_biz_obj_man_getBizObject: bizObjName
      call_biz_obj_man_getBizObject -left-> call_biz_obj_getOperationDefinition: bizAction
      call_biz_obj_getOperationDefinition -down-> [*]: fieldDef:GraphQLFieldDefinition
    }
  }

  state "GraphQLSelectionResolver~#resolveFieldSelection" as call_resolver_resolveFieldSelection {
    state "field#setFieldDefinition" as call_field_setFieldDefinition: 为字段 <b>field</b> 补充字段定义 <b>fieldDef</b>
    state "为 field 校验并补充\nGraphQL Field Argument 定义\nGraphQLArgumentDefinition" as validate_field_args
    state top_has_sub_field <<choice>>

    state "GraphQLEngine#getTypeDefinition" as call_engine_getTypeDefinition: 得到第一级 GraphQL Field Selection 的类型定义\n<b>objDef:GraphQLObjectDefinition</b>，其代表\nbizObjName 对应的业务对象的结构
    state "遍历 field 中的子级 GraphQL Field Selection" as get_field_from_field: 定义循环变量为 <b>subField:GraphQLFieldSelection</b>
    state "objDef#getField" as call_obj_def_getField: 获取 subField 的字段定义\n<b>subFieldDef:GraphQLFieldDefinition</b>
...
在 NopGraphQL 中不需要预先显式定义 GraphQL Schema，而对客户端提交的 GraphQL Document
的校验则是根据 `bizObjName` 对应的 [XMeta](../../manual/xmeta) 来动态处理的。
