...
例如，在 GraphQL Document 中定义一个 `OperationType` 为 `query` 的 `Operation`:
...
在 GraphQL Operation 中定义的 `SelectionSet` 自上而下构成了一棵树，
...
字段简单理解为<u>父字段取值结果</u>对象的属性名，而应该将其视为一个**字段取值函数**：
...
因此，对于前例中的 GraphQL Document，其取值过程如下：
...
最后，列举一些 `mutation` 类型的 GraphQL Document 样例，以便于了解其定义结构：
