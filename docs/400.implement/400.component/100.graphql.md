...
NopGraphQL 引擎为 Nop 平台默认的业务层实现方案，其负责向客户端提供与**业务对象**相关的**操作函数**，
并且通过 [NopORM 引擎](./orm)还可以实现对数据的增删改查操作。
...
[GraphQL Operation](https://spec.graphql.org/October2021/#sec-Language.Operations)（GraphQL 操作）
并按照给定的
[GraphQL Field](https://spec.graphql.org/October2021/#sec-Language.Fields)（GraphQL 字段）
结构对 GraphQL 操作的结果做**字段选择**（仅取指定字段的值）。
此外，NopGraphQL 也对 GraphQL 做了相关的增强，并去掉了一些无关的设计，
从而使得 GraphQL 能够更好地适应 Nop 平台的要求。

GraphQL Document 语法的主要结构如下：
...
上例中，在 GraphQL Operation 上定义了一个类型为
`QueryBeanInput`（其为 NopGraphQL 内置的
[GraphQL 输入类型](https://graphql.org/learn/schema/#input-types)）的变量
`$query`，该变量需由客户端随 GraphQL 端点调用一起传给服务端，
再由服务端将其转换（采用 JSON 反序列化等方式）为 `QueryBeanInput` 对象，
在 GraphQL Field 的参数列表中可以通过其变量名引用其值。

在 GraphQL Operation 中定义的 `SelectionSet` 为 GraphQL 字段的嵌套结构，
GraphQL Operation 的执行便是按该嵌套结构逐级做字段取值，
并最终返回与 GraphQL 层级字段具有相同结构的数据，比如：
...
可以看到，GraphQL 字段的结构与返回数据的属性结构是按层级而相互对应的，并且，GraphQL
字段的名字（`Name`）或别名（`Alias`）将会被用作为返回数据的属性名（字段别名优先于字段名）。

此外，在 GraphQL 字段上还可以指定参数，如 `roleList(limit: 10) { ... }` 上的参数 `limit`，
其作用是控制字段取值的结果，一般用于数据查询或者根据参数返回不同的值。
也就是，不能将 GraphQL 字段简单理解为数据的属性名，其代表的应该是一个**字段取值函数**：
取值时，需要将字段上的参数传给该字段的取值函数，再将取值结果赋值到返回数据的同名属性上。

字段取值函数增加了字段取值的灵活性和可扩展性，并且能够支持复杂的字段取值需求，
比如，调用外部接口获取所需的数据。

在 [graphql-java](https://www.graphql-java.com/documentation/data-fetching) 等
GraphQL 实现方案中，会在每个 GraphQL 字段的定义结构上绑定相应的取值函数（默认为数据对象的 getter 方法）。
...
因此，在前例中，可以这样理解 GraphQL 字段的取值过程：
...
- 继续向下取 `PageBean<UserEntity>` 的 `total` 和 `items` 属性的值，
  ...
  方法）后便结束字段取值，而 `items` 属性的类型为 `List<UserEntity>`，
  则需要继续对其做字段取值；
- 遍历 `items` 列表元素，依次对其各个元素做字段取值：分别获取列表元素
  ...
  不是标量类型，故而对其继续做字段取值；
- `roleList` 字段实际对应的字段取值函数是一个关联查询函数，用于查询与所属
...
只有对下一层级字段的取值是作用在其直接父级字段值的对象实例上的，因此，父子层级的字段取值存在引用关系。
例如，对上例中的 `items` 的子字段 `id`、`name` 取值，实际是取 `items` 字段值中的列表元素
...
由于在 GraphQL Operation 中的第一层级的字段，相互间没有关系，
其取值结果也不从属于某个具体的对象，故而，本文将 GraphQL Operation 的第一级字段称为
**GraphQL 根字段**。在一个 GraphQL Operation 内可以有多个**根字段**。

> 在 NopGraphQL 中需通过配置项 `nop.graphql.query.max-operation-count`
> 指定 GraphQL Operation 中允许出现的根字段的最多数量，缺省为 `10`。

此外，需要强调的是，GraphQL 是严格区分数据变更和数据查询操作的，不涉及数据变更的操作，
均使用 `query` 类型的 GraphQL Operation，而一切涉及到数据变更（增删改）的操作，
则必须都使用 `mutation` 类型的 GraphQL Operation，并且在 `query`
操作中不能做数据变更（当然，也没有措施严格保证这一行为），而在 `mutation`
操作中也一般不包含仅做数据查询的 GraphQL 字段，并且，通常也只包含一个用于变更调用的
GraphQL 字段。

> 在 `query` 内的多个 GraphQL 根字段可以并发取值，但在 `mutation`
> 中的多个 GraphQL 根字段只能按顺序执行，因为变更操作存在顺序性，前面的操作可能影响后面的操作。

下面列举一些 `mutation` 类型的 GraphQL Operation 样例，以便于了解其结构形式：
...
从以上的类图可以看到，NopGraphQL 的核心模型是被明确地分为了两个部分，
一个是 `IBizObject`，一个是 `GraphQLDocument`，
前者描述的是业务对象的组成结构，后者描述的是 GraphQL Document 的组成结构。
