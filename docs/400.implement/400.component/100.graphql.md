...
的，没有依赖于 Spring MVC、Hibernate 等具体的技术框架，
因此，只需要编写少量的 Web 端适配代码便可以支持除 GraphQL 以外的 REST、gRPC 等形式的 Web 调用，
并且，其在自动化测试、非 Web 应用开发以及与其他框架集成等方面，也具有更加灵活的优势。
...
- **GraphQL 操作字段**：在 GraphQL Document 中，取值函数被映射到**业务操作函数**的 GraphQL Field
  被称为**操作字段**；
- **GraphQL 对象类型**：其用于描述 GraphQL Document 中非标量类型字段的结构，
  从而限定 GraphQL 字段树的层级结构，确保 GraphQL Document 的有效性；
...
其会根据客户端给定的
[GraphQL Document](https://spec.graphql.org/October2021/#sec-Document) 执行
[GraphQL Operation](https://spec.graphql.org/October2021/#sec-Language.Operations) 并沿着
[GraphQL Field](https://spec.graphql.org/October2021/#sec-Language.Fields)（GraphQL 字段）的树形结构做
GraphQL 字段取值，再将字段取值结果返回给客户端。

> 但 NopGraphQL 的核心能力的是对以 DSL 模型进行设计和开发的业务对象的业务处理支持，
> 其对 GraphQL 的实现，主要是为了利用 GraphQL 的数据定制和组合能力。

GraphQL Document 的主要组成结构如下：
...
> 注意，虚线框节点表示其为可选节点，无需在 GraphQL Document 中定义。

根据以上 GraphQL Document 的组成结构，可以定义一个 `OperationType`
为 `query` 的 `Operation`:
...
在 GraphQL Operation 中定义的 `SelectionSet` 会自上而下形成一棵树，
本文称之为 **GraphQL 字段树**。因此，对 GraphQL Operation
的执行过程便是沿着**字段树**做**字段取值**的过程，其最终将返回与
GraphQL 字段树相一致的结构化数据，比如：
...
> 注意，作为返回数据的属性名，GraphQL 字段的别名（`Alias`）优先于其字段名（`Name`）。

而在 GraphQL 字段上指定的参数，例如 `UserEntity__findPage(query: $query)`
中的 `query` 和 `roleList(limit: 10)` 中的 `limit`，其作用是控制字段的取值结果，
一般用于数据查询或者根据参数返回不同的值。也就是，不能将 GraphQL
字段简单地理解为<u>父字段取值结果</u>对象的属性名，而应该将其视为一个**字段取值函数**：
对字段取值，就是调用与其绑定的取值函数，从而得到相应的字段取值结果。

> 在 GraphQL 字段的参数列表中，可以通过变量名（如 `$query`）引用在
> GraphQL Operation 上定义的变量（变量名需以 `$` 开头）。
> 而变量的实际值需要由客户端在提交 GraphQL Document 时一并回传给服务端，
> 回传数据的结构可参考[解析 GraphQL Document](#engine-parse-graphql-doc) 章节内容。
...
GraphQL 实现方案中，均会为每个 GraphQL 字段绑定相应的取值函数（缺省为调用对象属性的 getter 方法）。
在 GraphQL 字段取值的过程中，则会自上而下调用字段的取值函数，
直到遇到的字段取值结果为[标量类型](https://graphql.org/learn/schema/#scalar-types)时终止。

> **标量类型**：Scalar types，即整型、字符、布尔等非结构化数据。

因此，对于前例中的 GraphQL Document，其取值过程将经过以下几个步骤：

- 调用在 `UserEntity__findPage` 字段上绑定的取值函数，并向取值函数传入参数
  `query`，从而得到 `UserEntity` 的分页查询结果（在 NopGraphQL 中其类型为
  `PageBean<UserEntity>`）；
-
- 继续从 `PageBean<UserEntity>` 中取字段 `total` 和 `items` 对应的同名属性的值。
  由于 `total` 属性为标量类型，故而，在取得其值后便会结束该方向上的取值过程，而
  `items` 属性的类型为 `List<UserEntity>`，其不是标量类型，因此，继续对其做字段取值；
-
- 遍历 `PageBean#items` 列表元素，依次对其各个元素做字段取值：分别取出 `UserEntity`
  中字段 `id`、`name`、`roleList` 对应的属性值，而 `UserEntity#roleList`
  不是标量类型，故而，继续对其做字段取值；
-
- `UserEntity#roleList` 字段带有参数，因此，其实际绑定的是一个复杂的取值函数，
  这里假设该取值函数将根据参数 `limit` 查询出与当前 `UserEntity` 关联的前 `limit`
  条数据，并且，调用该字段取值函数后将得到分页查询结果 `PageBean<RoleEntity>`；
-
- 继续对 `UserEntity#roleList` 的分页结果做字段取值，直到遇到的字段值均为标量类型为止；
-
- 完成字段取值，组装并返回 GraphQL Document 的取值结果；

> 字段取值函数增加了字段取值的灵活性和可扩展性，可以实现比<u>对象属性取值</u>更为复杂的字段取值需求，
> 比如，调用外部接口获取字段数据。

在 GraphQL 字段取值的过程中，同一层级字段的取值是互不影响、互不相关的，
只有对子层级字段的取值是作用在其直接父级字段的取值结果上的，因此，父子层级的字段取值存在引用关系。
例如，对上例中的 `PageBean#items` 的子层级字段 `id`、`name` 取值，
实际是从 `PageBean#items` 字段值中的列表元素 `UserEntity` 中取字段 `id` 和 `name` 的值。

GraphQL 同级字段取值的独立性，也使得客户端能够在 GraphQL Operation 中同时指定多个字段，例如：
...
且其取值结果也不从属于某个具体的对象，故而，本文将 GraphQL Operation 中的第一层级的字段称为
**GraphQL 根字段**。而且，在一个 GraphQL Operation 内可以有多个**根字段**，
同样，也就存在多个以根字段为根节点的**字段树**。
...
均需定义 `query` 类型的 GraphQL Operation，而一切涉及到数据变更（增删改）的操作，
则都必须定义 `mutation` 类型的 GraphQL Operation，并且在 `query`
...
在对 [GraphQL](#about-graphql) 有一个基本的认识后，接下来，继续深入了解
NopGraphQL 引擎的核心设计与实现。
...
而后者则描述了业务数据的结构，主要由一组业务对象的**属性元数据**（由 `IObjPropMeta` 模型描述）组成。
...
> 分页查询结果 `PageBean<UserEntity>` 对应的 GraphQL 对象类型便是 `PageBean_UserEntity`。

对于任意层级的非标量类型的字段，在 GraphQL Document 中都必须指定其子字段，
且其子字段的字段名、字段类型也必须与其在 **GraphQL 对象类型**中定义的相同，
因此，根据 GraphQL 对象类型便可以实现对 GraphQL Document 的校验。

而在 NopGraphQL 中，则进一步要求 GraphQL 对象类型都必须定义相应的
[`IObjMeta`](#obj-meta)（**业务对象元数据**），而 `IObjMeta` 主要由
`IObjPropMeta`（**业务对象属性元数据**）列表组成，因此，NopGraphQL
可以自动推导并构造出由 `GraphQLObjectDefinition` 模型描述的 GraphQL 对象类型，
同时为其生成相应的字段定义 `GraphQLFieldDefinition` 列表，
并<u>不需要在开发时手工编写 GraphQL Schema</u>。
...
其中，GraphQL 根字段名，如 `UserEntity__findPage`，的结构必须是以双下划线 `__`
为分隔符的固定形式：在分隔符之前的部分对应的是业务对象名（对应
`BizObjectImpl#bizObjName`），如 `UserEntity`，而分隔符之后的部分对应的则是在
`IBizModel` 上定义的业务操作函数的名字，如 `findPage`，
并且，操作函数只能是被标注为 GraphQL `query` 或 `mutation` 的函数。

> `query` 函数由注解 `@BizQuery` 标注，而 `mutation` 函数则由注解 `@BizMutation`
> 标注。

也就是，NopGraphQL 是以<u>调用业务对象的查询或变更函数</u>为出发点进行 GraphQL 字段取值的，
...
和 `mutation` 函数收集起来，并构造为 GraphQL 操作字段定义 `GraphQLFieldDefinition`
单独记录到 `BizObjectImpl#operations` 中，而描述业务数据结构的 GraphQL 对象类型
...
但无论是根字段还是普通字段，其字段取值函数 `GraphQLFieldDefinition#fetcher` 均为
...
- `ServiceActionFetcher`: 调用 [`IBizModel`](#biz-model) 上的操作函数（如
  ...
  等）并得到其返回值，该返回值即为对应的操作字段的取值结果；
- `EvalActionDataFetcher`: 其封装的是在 `IBizModel` 中定义的 [@BizLoader](#biz-loader)
  数据加载函数，用于支持复杂的字段数据查询和批量查询等需求；

其中，`ServiceActionFetcher` 为 GraphQL 根字段的默认取值函数，而其他字段则一般缺省采用
...
```json
{
  ...
  // 并以去掉变量名开头的 `$` 字符的名字作为 variables 的属性名
  "variables": {
    "query": { ... },
    "id": 123456
  }
...
state "GraphQLSelectionResolver#resolveOperation(doc, op)" as call_resolver_resolveOperation {
  ...
  state "递归初始化字段：GraphQLSelectionResolver~#resolveFieldSelection(objName, fieldDef, field)" as call_resolver_resolveFieldSelection {
    ...
    state "GraphQLEngine#getTypeDefinition(objName)" as call_engine_getTypeDefinition: 得到业务对象 objName 的 GraphQL 对象类型定义\n<b>objDef:GraphQLObjectDefinition</b>（根据 IObjMeta 生成）
...
> 本章节不会对其校验过程进行展开分析，可以按照上图中的代码调用链查看方法的具体实现来了解其校验逻辑。
...
根字段的字段定义直接调用其对应业务对象 `bizObj` 的
`getOperationDefinition(GraphQLOperationType, String)` 方法即可获得。
...
`GraphQLEngine#getTypeDefinition(objName)` 得到当前字段的 **GraphQL 对象类型**定义 `objDef`，
然后，才能根据子字段名字 `subField.name` 从 `objDef`
中得到子字段的字段定义（对应调用 `objDef.getField(subField.name)`）。

在找到 GraphQL 字段的字段定义后，便可以将字段定义与其进行绑定（调用 `field.setFieldDefinition(fieldDef)`），
并可以继续进行字段参数的有效性校验等初始化工作。若不能为 GraphQL 字段找到其字段定义，则
GraphQL Document 必然不是有效的，直接抛出异常即可。

> 注意，根字段与其子字段的初始化过程是相同的。

若初始化过程正常结束，则继续进行 `GraphQLDocument` 字段取值。
...
的字段树调用字段定义上的取值函数 `GraphQLFieldDefinition#fetcher`，便可以得到与
GraphQL Document 相对应的取值结果。
...
只有从父字段的字段取值结果中，取出子字段对应的字段值（也就是对父字段的取值结果做**字段选择**），
并将这些子字段值组装后的数据才是最终返回给客户端的数据。
...
对 `GraphQLDocument` 的递归取值逻辑可用如下简化后的代码表示：

<!-- prettier-ignore -->
```java
public Promise<Object> fetchResult(
  List<GraphQLFieldSelection> fields, Object result
) {
  ...
  for (GraphQLFieldSelection field : fields) {
    ...
    promises.add(
      ...
      new Promise((resolve, reject) -> {
        // 得到父字段的取值结果。注意，取值结果也可能是 Promise
        Object fieldResult = fieldFetcher.get(env);

        Promise.resolve(fieldResult).then((result) -> {
          // 再递归取子字段值，得到的是做了字段选择的父字段选择结果
          Promise<Object> promise = fetchResult(
            field.getSelections(), result
          );
        ...
        // 保留字段名与字段选择结果的对应关系
        return new Object[]{ fieldDef.getName(), val };
      })
    );
  }
...
> 注意，以上为伪代码，并借用了 JavaScript 中的异步对象 `Promise`。
...
没有为其定义元数据。为此，NopGraphQL 对 `PageBean<?>` 做了专门的处理，以为其单独构造
`GraphQLObjectDefinition`，从而确保字段取值的一致性。

> 对 `PageBean<?>` 的 `GraphQLObjectDefinition` 的构造逻辑详见
> `ReflectionGraphQLTypeFactory#buildPageBeanType` 的实现。
> 此外，对于标注了 `@GraphQLObject` 的 Java Bean 也能够为其自动构造
> `GraphQLObjectDefinition`，其构造逻辑见
> `ReflectionGraphQLTypeFactory#newObjectDefinition`。

最后，NopGraphQL 会以 `GraphQLDocument` 的取值结果构造出 `GraphQLResponseBean`
后再返回给客户端：

```java
// 从 doc:GraphQLDocument 的根字段开始递归取值
fetchResult( doc.getRootFieldSelections(), null )
  .then((data) -> {
    // 组装客户端接收数据
...
而在 `GraphQLResponseBean#data` 属性上记录的便是 `GraphQLDocument`
的取值结果：
...
而通过 `IBizObject#objMeta` 和 `IBizObject#bizModel` 便可以分别构造出 GraphQL 对象类型定义
`IBizObject#objectDefinition` 以及 GraphQL 操作字段定义列表 `IBizObject#operations`。

以下为构造 `IBizObject` 的详细过程：
...
activate biz_obj_man
  ...
  activate biz_obj_builder
    ...
    activate biz_obj_builder
      ...
      == 将 IObjMeta 等价转换为 GraphQLObjectDefinition ==

      biz_obj_builder -> obj_meta_gql_def: 传参 IBizObject#objMeta
      activate obj_meta_gql_def
        obj_meta_gql_def -> obj_meta_gql_def: ~#toObjectDefinition(IObjMeta)

        activate obj_meta_gql_def
          obj_meta_gql_def -> obj_meta_gql_def: 构造 GraphQLObjectDefinition

          loop IObjPropMeta in IObjMeta
            obj_meta_gql_def -> obj_meta_gql_def: ~#toFieldDefinition(IObjPropMeta)

            activate obj_meta_gql_def
              obj_meta_gql_def -> obj_meta_gql_def: 根据 IObjPropMeta\n构造 GraphQLFieldDefinition

              obj_meta_gql_def -> obj_meta_gql_def: 返回 GraphQLFieldDefinition
            deactivate obj_meta_gql_def

            obj_meta_gql_def -> obj_meta_gql_def: GraphQLObjectDefinition\n添加 GraphQLFieldDefinition
          end

          obj_meta_gql_def -> obj_meta_gql_def: 返回 GraphQLObjectDefinition
        deactivate obj_meta_gql_def
        ...
        loop BizLoaderModel in IBizModel
          ...
          activate biz_model_gql_def
            ...
            activate biz_model_gql_def
              ...
              biz_model_gql_def -> biz_model_gql_def: GraphQLFieldDefinition\n绑定 EvalActionDataFetcher
      ...
      activate biz_obj_builder
        ...
        activate biz_obj_builder
          loop IBizActionModel in IBizModel
            ...
            biz_obj_builder -> biz_obj_builder: IBizObject 绑定操作字段\nGraphQLFieldDefinition\n到 IBizObject#operations
          end
...
因此，需要将其构造为 GraphQL 字段定义，并合并到 `IBizObject#objectDefinition` 中。
而 GraphQL 的操作字段则来自于在 `IBizModel` 上定义的 GraphQL `query` 和 `mutation` 函数，
需要在收集合并后单独赋值给 `IBizObject#operations`。
...
如果在 `IObjMeta` 中存在关联其他业务对象的属性 `IObjPropMeta`，
比如 [XMeta 对象关联配置](../../manual/xmeta#relations)，
则仅需以该业务对象的名字构造 `GraphQLNamedType`（调用 `GraphQLTypeHelper#namedType`），
并将其赋值到对应字段定义 `GraphQLFieldDefinition` 的 `type` 属性上即可，NopGraphQL
不会再对其结构做深度遍历。在做字段取值时，则会先根据字段关联的业务对象名字（对应调用
`fieldDef.getType().getNamedTypeName()`）得到其
`GraphQLObjectDefinition`，再根据其结构做子字段的取值。

> 可以查看 `ObjMetaToGraphQLDefinition#toGraphQLType` 的实现以了解对关联对象的构造逻辑。

## BizModel {#biz-model}

BizModel 的主要功能是向外部提供业务处理函数，实现对业务数据的增删改查。
本文将这些处理函数称之为**业务操作函数**，并细分为以下几类：
...
- **内部函数**：标注了 `@BizAction` 的函数，在启用了 maker checker 机制后，
  可用于检查对业务数据的变更操作是否合法等。详细说明见 [@BizAction](#biz-action)；
...
而若是返回的业务对象名与当前 BizModel 所属的业务对象的名字相同，则为 `bizObjName`
...
>   则对于返回该实体类型数据的操作函数，均不需要再标注 `@GraphQLReturn` 注解；
> - 在泛型的 BizModel 上定义的公共函数，如 Nop 内置的 `CrudBizModel<?>#findPage` 等，
>   则必须配置注解 `@GraphQLReturn`；
> - 操作函数可以在子线程内异步取值，只要返回值的类型为 `CompletionStage` 即可；

BizModel 可以不继承任何基类，也不需要实现任何 `interface`，仅需要在其 Java 类上标注
...
中配置相同的业务对象名，NopGraphQL 便会将它们的操作函数合并在一起（最终由 `IBizModel` 模型描述），
这便为以强类型方式定义的 BizModel 提供了基本的扩展能力：
...
> 有关 XBiz 模型的结构说明请查看参考手册 [XBiz 模型](../../manual/xbiz)。

XBiz 模型通过 xbiz 文件的名称来识别业务对象名字，如，`UserEntity.xbiz`
代表的是业务对象 `UserEntity` 的 XBiz 模型。同样，通过业务对象的名字，NopGraphQL
会将同名的 XBiz 模型上定义的操作函数和强类型方式定义的操作函数进行合并，
并且，XBiz 中的操作函数优先级是最高的，在 Java 类中定义的同名操作函数会被直接覆盖掉。

> XBiz 模型直接解析生成的便是 `IBizModel` 对象，二者本质上是同一个东西的不同表现形式。

XBiz 模型可以极大地增强了业务系统的可扩展性，例如，在 xbiz 文件中，仅通过如下方式便可引入逻辑编排：
...
> 之前，在 `GraphQLFieldDefinition#fetcher` 上已经设置了 `BeanPropertyFetcher`
> 以外的取值函数，则为同名字段定义的 `@BizLoader` 函数将被忽略，不会替换已经指定的取值函数。

`@BizLoader` 的作用是在 BizModel 上为指定的 GraphQL 字段提供定制的数据加载函数，
以实现一些复杂的数据加载逻辑，或者是数据的批量加载逻辑：
...
在 `@BizLoader` 函数上以 `@ContextSource` 标注的参数代表的是该定制字段的父字段的值，
也就是，该参数的类型必须是字段所属的业务对象的实体类，如 `UserEntity`，
或者以该实体类为泛型参数的 List 集合，如 `List<UserEntity>`。

假设 `@ContextSource` 标注参数是 `UserEntity` 类型，则在取字段 `roles` 的值时，
NopGraphQL 会挨个以 `UserEntity` 的实例为参数调用该字段的 `@BizLoader` 函数。
这在涉及对多个 `UserEntity` 的实例做字段选择时，会存在较大的性能问题，也即所谓的
[N + 1 问题](https://juejin.cn/post/6923552292197105677)。

一般情况下，为了解决性能问题，需要采用批量加载机制。此时，可以直接将 `@ContextSource`
标注的参数定义为 `List<UserEntity>` 类型，NopGraphQL 将会自动采用
[DataLoader](https://www.graphql-java.com/documentation/batching/)
机制进行批量加载。

例如，在以下的 GraphQL Document 中有多个 `roles` 字段均需要调用对应的
`@BizLoader` 函数：
...
因此，可以将 `roles` 字段的 `@BizLoader` 函数定义为以下形式：
...
public class UserEntityBizModel {
  ...
  public List<List<RoleEntity>> roles(
    ...
    // 再将每个 roles 按照与 user 在 users 中相同的顺序插入到返回结果列表中
  }
}
```

> 返回结果必须是 `List` 类型，且其中的元素位置和元素总量必须与
> `@ContextSource` 参数中的元素保持一致。

在进行 GraphQL 字段取值时，NopGraphQL 将按字段的加载顺序依次收集所有
`roles` 字段的父字段的值到列表 `List<UserEntity>` 中，再将收集到的
`UserEntity` 列表传递给 `@BizLoader` 函数以批量加载 `RoleEntity`。
...
将根据参数中 `UserEntity` 的顺序从结果列表中抽取出 `List<RoleEntity>`。
...
函数用以调用对应的 `@BizLoader` 函数，再以 `BatchLoader` 创建一个 `DataLoader`
实例 `loader`，并将其注册到 GraphQL 取值的上下文 `IGraphQLExecutionContext`
中以便于复用，最后，执行方法 `loader.load(...)` 以返回加载结果：

```java
public Object get(IDataFetchingEnvironment env) {
  IGraphQLExecutionContext context = env.getGraphQLExecutionContext();
  // Note: 在此例中 loaderName
  // 为 io.nop.demo.service.entity.UserEntityBizModel@roles
  ...
  if (loader == null) {
    ...
    BatchLoader<Object, Object> batchLoader = keys -> {
      // 在 @ContextSource 所标注的位置（对应变量 sourceIndex）
      // 赋值 List<UserEntity>
...
需要注意的是，`loader.load(...)` 返回的是一个 Promise 异步对象，
这是一种数据异步、延迟加载机制，
可以确保只有在源数据收集完毕后才真正开始对目标数据的批量加载。

下面先对 Promise 的工作原理做一个简单说明，再解释 `DataLoader`
是如何起到收集和结果再组织的作用的。

Promise 为异步操作最终完成或失败的对象，用于解决异步回调函数嵌套的问题，
在 Java 中，该对象为 `CompletableFuture`。关于 Promise 需要明白以下几点：
...
- 若要让 Promise 执行结束以触发后续的链式调用，则需要在持有其实例的线程中显式调用
  ...
  等方法，并回传处理结果或处理异常。也可以在当前线程内，其他持有该 Promise
  实例的位置调用 `CompletableFuture#complete`；

基于以上几点基础知识，再来详细分析 `DataLoader` 批处理的实现逻辑。

以下面的 GraphQL Document 为例（省去了无关的字段）：
...
表示 Promise 对象，$U_{r0}, U_{r1}, U_{r2}, U_{r...}, U_{rn}$ 表示与
Promise 对象 $P_{r0}, P_{r1}, P_{r2}, P_{r...}, P_{rn}$ 相对应的
`UserEntity` 实例，也就是，每一个 `UserEntity` 实例均会对应唯一的 Promise 对象。
...
字段取值函数 `BeanMethodBatchFetcher` 每次被调用时，都会调用唯一绑定的
`DataLoader` 的 `load(Object)` 方法，其经过方法调用链
`DataLoader#load -> DataLoaderHelper#load -> DataLoaderHelper#queueOrInvokeLoader`
会为入参构造唯一的 Promise 对象（即上图中的
$P_{r0}, P_{r1}, P_{r2}, P_{r...}, P_{rn}$）并与入参（即上图中的
$U_{r0}, U_{r1}, U_{r2}, U_{r...}, U_{rn}$）进行绑定，再返回给 $P_2, P_3$
以继续后续 Promise 调用。

`DataLoader#load` 的入参为 $P_2, P_3$ 的取值结果，其中，$P_3$ 的结果为
`List<UserEntity>`，所以，会为该 `List` 中的每个 `UserEntity` 都构造一个
Promise 对象，也即 $P_{r1}, P_{r2}, P_{r...}, P_{rn}$。

由于 $P_1, P_2, P_3$ 处的取值函数返回的都是直接数据（非 Promise 对象），
所以，在得到根 Promise $P_0$ 后，$P_{r0}, P_{r1}, P_{r2}, P_{r...}, P_{rn}$
也已经构建完成了，只是它们还在等待各自 `UserEntity#roles` 的取值结果。

对 GraphQL Document 的字段取值是在方法 `GraphQLExecutor#executeAsync` 中进行的，
其在调用 `GraphQLExecutor#invokeOperations` 方法得到 $P_0$ 后，
便会调用 `IGraphQLExecutionContext#dispatchAll` 以触发对 `DataLoader`
的批量加载函数的调用。

`IGraphQLExecutionContext#dispatchAll` 会依次调用在其内部注册的 `DataLoader`
实例的 `dispatch()` 方法，并最终由 `DataLoaderHelper#dispatchQueueBatch`
执行在 `DataLoader` 上绑定的批量加载函数 `BatchLoader`。

`DataLoaderHelper#dispatchQueueBatch` 会将收集到的
$U_{r0}, U_{r1}, U_{r2}, U_{r...}, U_{rn}$ 组装成 `List<UserEntity>`
后，作为参数传给 `BatchLoader#load` 并得到 `@BizLoader` 函数返回的批量加载数据列表
`List<List<RoleEntity>>`。

接着，依次遍历 `List<List<RoleEntity>>` 得到相同位置的 `UserEntity#roles`
所对应的取值结果，再调用与各个 `UserEntity` 绑定的 Promise
对象（$P_{r0}, P_{r1}, P_{r2}, P_{r...}, P_{rn}$）的
`CompletableFuture#complete` 方法结束 Promise，并同时将各自的 `UserEntity#roles`
传递给后续的链式调用函数。
...
在批处理参数收集完毕后，调用 `DataLoader#dispatch` 便会执行真正的批量加载函数，
并将得到的结果按顺序传给各个 Promise 对象，从而继续后续的字段取值操作。

需要注意的是，当前 NopGraphQL 的实现中，并未处理父字段的取值结果为 Promise 的情况，
也就是，如果 `DataLoader` 的收集发生在异步线程执行之后，那么，仅在
`GraphQLExecutor#invokeOperations` 之后调用 `GraphQLExecutor#dispatchAll`
将无法主动结束在异步线程之后创建的 Promise，进而造成接口无响应，
其会一直等待异步线程之后的 Promise 的结束。
...
| 参数类型 | 必须的参数注解 | 说明 |
| ---- | --- | --- |
| `IServiceContext` 及其子类 |  | GraphQL 服务请求的上下文 |
| `ParsedGraphQLRequest` | `@ContextRoot` | 解析后的 GraphQL 请求数据，包含 `GraphQLDocument`（`ParsedGraphQLRequest#document`）以及 GraphQL 变量列表（`ParsedGraphQLRequest#variables`）。也可通过 `IServiceContext#getRequest()` 获取 |
| `IContext` |  | BizModel 操作函数执行上下文。也可通过 `IServiceContext#getContext()` 获取 |
| `IEvalScope` 及其子类 |  | Xpl 脚本执行的作用域。也可通过 `IServiceContext#getEvalScope()` 获取 |
| `IUserContext` 及其子类 |  | 用户上下文。也可通过 `IServiceContext#getUserContext()` 获取 |
| `ICache` 及其子类 |  | 用于缓存字典、关联对象等数据。也可通过 `IServiceContext#getCache()` 获取 |
| `FieldSelectionBean` 及其子类 |  | GraphQL 根字段的结构，含参数、子字段等信息 |
| `ApiRequest` |  | - `ApiRequest#data`: 对应 GraphQL 根字段的所有参数组成的 Java Bean；<br/>- `ApiRequest#selection`: 与当前操作函数中的 `FieldSelectionBean` 类型参数相同；<br/>- `ApiMessage#headers`: 来自于 `IServiceContext#getRequestHeaders()`； |
| 不定 | `@RequestBean` | 若 GraphQL 根字段参数为 `String` 类型，且 `@RequestBean` 标注的参数类型使用了类注解 `@DataBean`，则该参数对应的便是 GraphQL 根字段参数反序列化后的对象 |
| 不定 | `@Name` | 通过参数名映射在当前 GraphQL 字段上的同名参数。参数名由 `@Name` 的 `value` 确定，若未设置该值，则采用参数的定义名。除前面已经列出的操作函数的参数外，其余参数都必须使用该注解，与注解 `@Optional`/`@Nullable` 配合，还可以指定在当前 GraphQL 字段上的同名参数是否为可选或为 `null`，缺省要求是参数必须存在且不为 `null` |
...
| 参数类型 | 必须的参数注解 | 说明 |
| ---- | --- | --- |
| `IDataFetchingEnvironment` 及其子类 |  | `@BizLoader` 字段的取值函数的取值环境 |
| 不定 | `@ContextRoot` | 对应 GraphQL 根字段的取值结果 |
| 不定 | `@ContextSource` | 对应 `@BizLoader` 字段的父字段的取值结果。**至多只能定义一个** |
| 不定 | `@RequestBean` | 对应 `@BizLoader` 字段的所有参数组成的 Java Bean |
| `IServiceContext` 及其子类 |  | GraphQL 服务请求的上下文。也可通过 `IDataFetchingEnvironment#getServiceContext()` 获取 |
| `IContext` |  | BizModel 操作函数执行上下文。也可通过 `IDataFetchingEnvironment#getContext()` 获取 |
| `IEvalScope` 及其子类 |  | Xpl 脚本执行的作用域。也可通过 `IDataFetchingEnvironment#getEvalScope()` 获取 |
| `IUserContext` 及其子类 |  | 用户上下文。也可通过 `IDataFetchingEnvironment#getUserContext()` 获取 |
| `ICache` 及其子类 |  | 用于缓存字典、关联对象等数据。也可通过 `IDataFetchingEnvironment#getCache()` 获取 |
| `FieldSelectionBean` 及其子类 |  | `@BizLoader` 字段的结构，含参数、子字段等信息 |
| `ApiRequest` |  | - `ApiRequest#data`: 与当前字段加载函数中 `@RequestBean` 标注的参数相同；<br/>- `ApiRequest#selection`: 与当前字段加载函数中的 `FieldSelectionBean` 类型参数相同；<br/>- `ApiMessage#headers`: 来自于 `IServiceContext#getRequestHeaders()`； |
| 不定 | `@Name` | 通过参数名映射在当前 `@BizLoader` 字段上的同名参数。参数名由 `@Name` 的 `value` 确定，若未设置该值，则采用参数的定义名。除前面已经列出的加载函数的参数外，其余参数都必须使用该注解，与注解 `@Optional`/`@Nullable` 配合，还可以指定在当前 `@BizLoader` 字段上的同名参数是否为可选或为 `null`，缺省要求是参数必须存在且不为 `null` |
...
NopGraphQL 支持在 Java Bean 的属性上以标注 `@PropMeta` 的方式声明内置模型的元数据。
但针对业务对象，则一般采用领域模型 XMeta 来定义业务对象的元数据。

> XMeta 模型直接解析生成的便是 `IObjMeta` 对象，二者本质上是同一个东西的不同表现形式。
...
> 有关 XMeta 模型的结构说明请查看参考手册 [XMeta 模型](../../manual/xmeta)。
