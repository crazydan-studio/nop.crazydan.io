...
- **业务对象**：描述应用**业务数据**的结构和处理逻辑的对象，称为业务对象；
- **业务操作函数**：针对**业务数据**进行业务处理的函数，在 Java 中，其为定义在类上的方法，
...
[GraphQL Operation](https://spec.graphql.org/October2021/#sec-Language.Operations) 并按照给定的
[GraphQL Field](https://spec.graphql.org/October2021/#sec-Language.Fields)（GraphQL 字段）嵌套结构进行
...
而在 GraphQL Operation 中定义的 `SelectionSet` 为 GraphQL 字段的嵌套结构，GraphQL Operation
的执行便是按该嵌套结构逐级做**字段取值**，并最终返回与 GraphQL 层级字段具有相同结构的数据，比如：
...
字段的名字（`Name`）或别名（`Alias`）将会被用作为返回数据的属性名（字段的别名优先于字段名）。
...
只是服务端与客户端进行业务数据和业务操作的一种交互形式，其本质只是业务对象的一种表现方式。

业务对象 `IBizObject` 主要由 `IBizModel`（**业务模型**）和 `IObjMeta`（**业务对象元数据**）组成。
前者定义了一组与业务对象相关的**业务操作函数** `actions`（由 `IBizActionModel` 模型描述），
以及一组与 GraphQL 字段相对应的字段取值函数 `loaders`（由 `BizLoaderModel` 模型描述）；
而后者则描述了业务数据的结构，主要为业务数据的属性（由 `IObjPropMeta` 模型描述）集合。
其中，<u>`IObjMeta` 不是必须的</u>，因为，业务数据可能只是简单类型，并没有明确的结构，
但 <u>`IBizModel` 则是必须的</u>，因为，不需要做任何操作的业务对象，是不需要由应用系统管理的，
其存在没有意义。

GraphQL Document 为客户端所提供的 GraphQL 文本内容，其解析后由 `GraphQLDocument` 模型描述。
...
`GraphQLFieldSelection` 为嵌套定义结构，持有子层级的 `GraphQLFieldSelection`
字段列表，因此，能够完整描述 GraphQL Document 的树形结构。

传统的 GraphQL 在使用前，还必须定义相应的 GraphQL Schema（GraphQL 模式），以用于校验客户端提交的
GraphQL Document 的有效性。而 NopGraphQL 则会根据 `IBizModel` 和 `IObjMeta` 自动推导并生成
GraphQL Schema，并不需要手工编写。

在 GraphQL Schema 中定义的是可操作的业务对象的结构，该结构由 `GraphQLObjectDefinition`
模型描述，其包含的主要是 GraphQL 的字段定义 `GraphQLFieldDefinition` 列表，而
`GraphQLFieldDefinition` 则是根据 `IObjMeta` 所持有的
`IObjPropMeta`（**业务对象属性元数据**）生成，并且在 `GraphQLFieldDefinition`
还会与相应的字段取值函数 `IDataFetcher` 绑定，用以支持一些自定义的字段取值需求，
但一般不需要为 GraphQL 字段定义绑定 `IDataFetcher`，NopGraphQL 默认会采用对象属性的
getter 方法作为取值函数。

`IBizObject#operations` 是什么？`GraphQLFieldSelection` 怎么与 `GraphQLFieldDefinition` 绑定？
