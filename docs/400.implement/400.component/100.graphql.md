...
方法也标注了注解 `@PropMeta`，可以将其视为以强类型方式定义的 [XMeta](#xmeta) 结构。

> 对 `PageBean` 的结构分析逻辑见 `ReflectionGraphQLTypeFactory#buildPageBeanType`。

NopGraphQL 限定只有定义了 [XMeta](#xmeta) 结构的对象才能做字段取值，而对此类对象的字段取值，
则默认采用 `BeanPropertyFetcher` 函数，其会直接调用对象上的 getter 方法（如 `UserEntity#getId()`），
从而取得与字段同名的对象属性的值。

> 默认字段取值函数的确定逻辑见 `GraphQLExecutor#hookFetch`。

除此以外，还可以在 BizModel 的函数上标注 `@BizLoader`（数据加载器），NopGraphQL
会将此类**数据加载函数**封装在字段取值函数 `EvalActionDataFetcher` 中，
在字段取值过程中遇到与 `@BizLoader` 标注的名字相同的字段时，
则最终会调用 BizModel 的数据加载函数获取字段值。
...
`IBizModel` 由业务操作函数（由 `IBizActionModel` 表达）和数据加载函数（由
...
activate biz_obj_man
  ...
  activate biz_obj_builder
    ...
    activate biz_obj_builder
      ...
      == 向 GraphQLObjectDefinition 补充数据加载器字段 ==
      ...
      activate biz_obj_builder
        activate biz_obj_builder
          loop BizLoaderModel in IBizModel
            ...
            biz_obj_builder -> biz_obj_builder: GraphQLObjectDefinition\n绑定数据加载器字段\nGraphQLFieldDefinition
          end
...
- **数据加载函数**：标注了 `@BizLoader` 的函数，其只用于为定义在 XMeta
  中的同名属性提供自定义的、应对复杂需求的数据加载支持，从而提供更为强大和灵活的 GraphQL 字段取值能力；
...
<!--
- `@BizAction` 标注的函数用于 `<maker-checker />`；
- `@GraphQLReturn(bizObjName = BIZ_OBJ_NAME_THIS_OBJ)` 用于指定操作函数的返回值对应的业务对象名字
  `bizObjName` 与当前 BizModel 对应的业务对象的名字相同；
  - `BizModelToGraphQLDefinition#toOperationDefinition`
    - `BizModelToGraphQLDefinition#getType`
      - `ObjMetaToGraphQLDefinition#toGraphQLType(String, ISchema, boolean, TypeRegistry, boolean)`
        - `ReflectionGraphQLTypeFactory#buildGraphQLType(IGenericType, String, String, TypeRegistry, boolean)`
...
- `io.nop.biz.impl.BizObjectBuilder`
- `BizObjectBuilder#buildBizLoaders`
  - `ReflectionBizModelBuilder#build`
