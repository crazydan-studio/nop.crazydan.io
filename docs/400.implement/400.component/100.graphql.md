...
`GraphQLObjectDefinition`，再根据 `GraphQLObjectDefinition` 的结构继续递归初始化
`GraphQLDocument` 的子字段。
...
> 有关 XBiz 模型的结构说明请阅读参考手册[《XBiz 模型》](../../manual/xbiz)。
...
> 在[《对 GraphQLDocument 做字段选择》](#engine-select-graphql-field)章节中可查看其代码实现逻辑。
...
有关 XMeta 模型的结构说明请阅读参考手册[《XMeta 模型》](../../manual/xmeta)。
