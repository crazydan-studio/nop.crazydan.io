...
```plantuml
actor "客户端" as client
control "解析器" as resolver
component "GraphQLEngine" as engine
component "IBizModel" as biz
control "序列化" as serializer
component "IObjMeta" as meta

client -down-> resolver: 请求 Body：\l{\lquery: 'query Xxx__findPage(...) { ... }',\lvariables: { ... }\l}
resolver -down-> engine: GraphQLDocument

engine -up-> serializer: GraphQLResponseBean
serializer -up-> client: 响应 Body：\l{ data: {\lXxx__findPage: { ... }\l} }

engine -down-> biz: 调用业务处理函数
biz -up-> engine: 返回处理结果
