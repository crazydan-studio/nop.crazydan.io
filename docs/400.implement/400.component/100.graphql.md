...
其中，`{bizObjName}` 表示 [BizModel](#biz-model) 的名字 `UserEntity`，
而 `{bizAction}` 则表示在 `{bizObjName}` 上定义的操作函数 `findPage`。

对 `GraphQLDocument` 的初始化也将按照以上要求的形式，从对第一级选择字段名字的分解开始：
...
engine -> engine: 调用\n~#initDocument(\n  doc:GraphQLDocument\n)
activate engine
  engine -> engine: 调用\n~#resolveSelections(doc)
  activate engine
    engine -> resolver: 传参 doc
    activate resolver
      resolver -> resolver: 调用\n~#resolveSelection(doc)
      activate resolver
        resolver -> resolver: 从 doc 获取\nop:GraphQLOperation
        activate resolver
          resolver -> resolver: 调用\n~#resolveOperation(\n  doc, op\n)
          activate resolver
            loop op 中的 field:GraphQLFieldSelection
              resolver -> engine:
