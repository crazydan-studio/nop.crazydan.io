...
以上查询函数除 `#findRoots` 外，均会通过 `TreeEntityHelper#buildTreeEntityBaseSql`
构造以下形式的 Recursive <NopDocLink title="EQL" path="/dev-guide/orm/eql.md" />
查询语句以对业务对象树做过滤查询：
...
> 详细说明见[《GraphQL 指令：@TreeChildren》](#graphql-directive-tree-children)。

不过，该方式在查询深度较大且子节点数量较多时会存在比较明显的性能问题，需要权衡后再选择合适的方案。
