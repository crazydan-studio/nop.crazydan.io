...
用于定义业务对象属性对应的 GraphQL 字段的**参数项**，例如：
...
[xpl-fn](./dict/std-domain#opt_xpl_fn):(entity,args,propMeta)=>any

</TCol><TCol> 属性的 getter 函数 </TCol>
<TCol> 否 </TCol><TDesc>
...
[xpl-fn](./dict/std-domain#opt_xpl_fn):(entity,value,propMeta)=>void
...
再逐条进行子查询，因此可能会出现明显的性能问题，在性能问题较明显时，需考虑在 <NopDocLink title="@SqlLibMapper" path="/dev-guide/orm/sql-lib.md" />
中做自定义查询或者采用按需加载机制。
...
- [`graphql:queryMethod`](#attr_props_prop_graphql_queryMethod): ;
- [`graphql:connectionProp`](#attr_props_prop_graphql_connectionProp): ;
- [`graphql:authObjName`](#attr_props_prop_graphql_authObjName): ;
- [`graphql:maxFetchSize`](#attr_props_prop_graphql_maxFetchSize): ;
- [`<graphql:orderBy />`](#attr_props_prop_graphql_orderBy): 始终附加的排序条件;
- [`<graphql:filter />`](#attr_props_prop_graphql_filter): 始终附加的过滤条件;
...
## GraphQL 字段参数项定义 {#def_attr_props_prop_arg}
