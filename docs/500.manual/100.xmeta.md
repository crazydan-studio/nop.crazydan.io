...
用于设置属性是否为内部属性。缺省为 `false`。

被标记为内部属性的属性在 XView 生成的页面中默认不会为其创建对应的组件。
但并不影响其作为 GraphQL 的选择字段，仅
[published](#attr_props_prop_published) 影响 GraphQL 字段的可见性。
...
用于设置属性是否已被废弃，已废弃的属性将不再被使用。缺省为 `false`
...
用于设置在新增业务对象数据时，是否可插入该属性的值。缺省为 `false`
...
用于设置在更新业务对象数据时，是否可更新该属性的值。缺省为 `false`
...
GraphQL Document 中，否则将报字段未定义的异常。缺省为 `true`。
...
> 相关处理逻辑见 `BizExportHelper#isAllowExport`。目前未真正使用。
...
变量中指定根据当前属性做 ORM 排序，例如：

```xml title="XMeta 定义"
    <prop name="status" sortable="true" />
```

```json {5} title="GraphQL 调用的回传数据"
{
  "query": "query ($query: QueryBeanInput) { ... }",
  "variables": { "query": {
    "orderBy": [{
...
<TCol> boolean </TCol><TCol> 是否可查询属性 </TCol>
<TCol> 是 </TCol><TDesc>
