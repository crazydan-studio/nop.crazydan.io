...
用于设置与 ORM
实体的[唯一键](./xorm#attr_entities_entity_unique_keys_unique_key)相映射的[属性名](#attr_props_prop_name)。

在新增或修改 ORM 实体对象时，将自动检查该列表中各个属性的值在数据库中是否存在重复。校验逻辑详见
...
用于设置在查询该 ORM 实体时需始终附加的过滤条件，追加该过滤条件的逻辑见
`CrudBizModel#prepareFindPageQuery`。

此外，在获取、删除、更新、复制某个确定的 ORM 实体对象时，NopGraphQL
也会检查该实体对象是否满足该过滤条件，其检查逻辑见 `CrudBizModel#checkMetaFilter`。
...
用于设置在查询该 ORM 实体时需始终附加的排序条件，追加该排序条件的逻辑见
`CrudBizModel#prepareFindPageQuery`。
...
用于配置 ORM 实体的树形结构信息。NopGraphQL 将根据此结构查询和维护树形结构的
