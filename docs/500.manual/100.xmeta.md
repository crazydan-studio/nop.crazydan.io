...
映射该组件内部的数据 `funcMetaComponent.args`，由于无法自动推导出该数据的类型，
因此，需要显式设置 `graphql:type` 为 `[Map]`，从而确保返回给客户端的
`args` 的字段值为 JSON 对象形式，而不是字符串。

> 对 GraphQL 字段的取值结果做类型转换的逻辑见 `GraphQLExecutor#normalizeValue`。

该配置项的可选值为枚举类 `GraphQLScalarType` 中的各个枚举项的名字（大小写敏感），如
`String`、`Map` 等。若 GraphQL 字段为集合类型，则需要将可选值包裹在 `[ ]`
内，如 `[String]`（字符串列表）、`[Map]`（Map 列表）。

默认通过 [&lt;schema />](#attr_props_prop_schema) 来确定业务对象属性所对应的
GraphQL 字段的 GraphQL 类型，但对于 ORM JSON 组件内部的数据，则一般通过
`graphql:type` 指定其 GraphQL 类型，且 `graphql:type` 的优先级高于
`<schema />`。在二者均未配置的情况下，缺省采用 `String` 类型。
...
型数据，这时，便需要设置 `graphql:type="String"`。

但在做 GraphQL 变更时，
客户端提供的输入数据则需由相应的[业务操作函数](../implement/component/graphql#biz-model)进行转换处理，
如 `CrudBizModel#doSave`，NopGraphQL 不会按照业务对象属性的类型对输入数据做主动转换。

> 对 ORM 实体对象属性的输入转换逻辑见 `DynamicOrmEntity#orm_propValue(int, java.lang.Object)`。
...
</TCol><TCol> 作为业务对象属性显示文本的属性名 </TCol>
<TCol> 否 </TCol><TDesc>

在业务对象属性指向的是对象（含对象集合）或数据字典时，
...
在业务对象属性的数据类型为 `Timestamp`，且系统配置
`nop.graphql.ignore-millis-in-timestamp`（`Timestamp` 类型是否不以毫秒形式返回）为 `true`
时，NopGraphQL 引擎会自动将业务对象属性的值按该设定的形式格式化为字符串后返回。
...
在业务对象属性上做过滤运算的过滤器均会被替换为该转换函数所构造的过滤器，从而简化复杂过滤条件的使用。
...
函数对业务对象属性的值做掩码处理，如，`ui:maskPattern="3*4"` 表示保留前 3 位以及后 4 位字符，
...
Nop 模块标识（如，`nop/auth`）。在业务对象属性关联的是其他模块的对象时，需要通过该属性指示该关联对象所属的模块
...
用于自动生成订单号、卡号等，该值不为空时，将在 `x:post-extends` 阶段为业务对象属性自动生成
