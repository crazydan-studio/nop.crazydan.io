...
其优先级高于 [ext:joinRightDisplayProp](#attr_props_prop_schema_ext_joinRightDisplayProp)。
详细说明见 [XMeta 属性显示文本](#def_attr_props_prop_schema_graphql_labelProp)
...
仅用于 [graphql:labelProp](#attr_props_prop_schema_graphql_labelProp)
...
阶段为<b>当前属性</b>自动生成 `transformOut`（详见 Xpl 函数 `meta-gen:GenMaskingExpr`）
...
**[XView Only]**
在<b>当前属性</b>为 JSON 文本时会自动构造一个 [&lt;schema />](#attr_props_prop_schema_schema) 为
...
## XMeta 属性显示文本 {#def_attr_props_prop_schema_graphql_labelProp}
...
`relatedRoleList_label`，而在 [Nop Orm](/manual/orm) 层中将会在实体对象的
...
> - `graphql:labelProp` 的配置说明见 [XMeta 属性显示文本](#def_attr_props_prop_schema_graphql_labelProp)；
> - 非必要情况，对关联目标对象的加载方式默认均为懒加载，即，`lazy="true"`；
