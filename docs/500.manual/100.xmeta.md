...
<TRow><TCol id="attr_props_prop_schema_name"> name </TCol>
...
<TRow><TCol id="attr_props_prop_schema_schema"> schema </TCol>
<TCol> ISchema </TCol><TCol> 属性 Schema </TCol>
<TCol> 否 </TCol><TCol>

对当前属性值类型、值精度等的约束定义。详细说明见 [XMeta 属性 Schema](#def_attr_props_prop_schema)

</TCol></TRow>

<!-- -->

<TRow><TCol id="attr_props_prop_schema_displayName"> displayName </TCol>
...
<TRow><TCol id="attr_props_prop_schema_description"> description </TCol>
...
<TRow><TCol id="attr_props_prop_schema_defaultValue"> defaultValue </TCol>
...
<TRow><TCol id="attr_props_prop_schema_propId"> propId </TCol>
...
<TRow><TCol id="attr_props_prop_schema_mapToProp"> mapToProp </TCol>
<TCol> prop-path </TCol><TCol> 映射到 ORM 实体的指定属性 </TCol>
<TCol> 否 </TCol><TCol>

也就是为 ORM 实体的属性（注意，**不是当前对象的属性**）映射一个别名，
...
<TRow><TCol id="attr_props_prop_schema_depends"> depends </TCol>
<TCol> csv-set </TCol><TCol> 当前属性所依赖的属性列表 </TCol>
<TCol> 否 </TCol><TCol>

逗号分隔的属性列表（需在 ORM 实体上有映射）。在获取当前属性的值时，需要依赖其他字段。
例如，在批量加载当前属性时，需要把依赖的属性也进行批量加载

</TCol></TRow>

<!-- -->

<TRow><TCol id="attr_props_prop_schema_mandatory"> mandatory </TCol>
<TCol> boolean </TCol><TCol> 属性值必填？ </TCol>
<TCol> 是 </TCol><TCol>

指示当前属性是否为必填项。默认为 `false`

</TCol></TRow>

<!-- -->

<TRow><TCol id="attr_props_prop_schema_internal"> internal </TCol>
<TCol> boolean </TCol><TCol> 是否内部属性？ </TCol>
<TCol> 是 </TCol><TCol>

内部属性一般仅在后端使用，对前端不可见。默认为 `false`

</TCol></TRow>

<!-- -->

<TRow><TCol id="attr_props_prop_schema_deprecated"> deprecated </TCol>
...
<TRow><TCol id="attr_props_prop_schema_insertable"> insertable </TCol>
...
<TRow><TCol id="attr_props_prop_schema_updatable"> updatable </TCol>
...
<TRow><TCol id="attr_props_prop_schema_virtual"> virtual </TCol>
...
<TRow><TCol id="attr_props_prop_schema_published"> published </TCol>
...
<TRow><TCol id="attr_props_prop_schema_exportable"> exportable </TCol>
<TCol> boolean </TCol><TCol> 是否可导出？ </TCol>
<TCol> 是 </TCol><TCol>

仅当该值为 `true` 且
[published](#attr_props_prop_schema_published) 也为 `true`
...
<TRow><TCol id="attr_props_prop_schema_sortable"> sortable </TCol>
...
<TRow><TCol id="attr_props_prop_schema_queryable"> queryable </TCol>
...
<TRow><TCol id="attr_props_prop_schema_allowFilterOp"> allowFilterOp </TCol>
...
<TRow><TCol id="attr_props_prop_schema_lazy"> lazy </TCol>
...
<TRow><TCol id="attr_props_prop_schema_ext_kind"> ext:kind </TCol>
<TCol> string </TCol><TCol> </TCol>
<TCol> 否 </TCol><TCol>

可选值：

- `to-one`：当前对象与属性对应的对象为一对一关联；
- `to-many`：当前对象与属性对应的对象为一对多关联；
...
<TRow><TCol id="attr_props_prop_schema_tagSet"> tagSet </TCol>
...
<TRow><TCol id="attr_props_prop_schema_graphql_type"> graphql:type </TCol>
<TCol> string </TCol><TCol> 当前对象属性值的返回类型 </TCol>
<TCol> 否 </TCol><TCol>

将当前对象属性的值按该设定的类型转换后返回。该属性的可选值为枚举类
...
<TRow><TCol id="attr_props_prop_schema_graphql_mapper"> graphql:mapper </TCol>
<TCol> string </TCol><TCol> </TCol>
<TCol> 否 </TCol><TCol>
...
<TRow><TCol id="attr_props_prop_schema_graphql_labelProp"> graphql:labelProp </TCol>
<TCol> string </TCol><TCol> 作为当前属性显示文本的属性名 </TCol>
<TCol> 否 </TCol><TCol>

在当前属性是一个 ORM 关联对象或数据字典时，
需要指示用当前对象中的哪个[属性](#def_attr_props_prop)的值作为
[关联对象](#relations-many-to-many) 或数据字典的显示文本。其优先级高于
`ext:joinRightDisplayProp`

</TCol></TRow>

<!-- -->

<TRow><TCol id="attr_props_prop_schema_graphql_joinLeftProps"> graphql:joinLeftProps </TCol>
...
<TRow><TCol id="attr_props_prop_schema_graphql_joinRightProps"> graphql:joinRightProps </TCol>
<TCol> word-set </TCol><TCol> </TCol>
<TCol> 否 </TCol><TCol>
...
<TRow><TCol id="attr_props_prop_schema_graphql_datePattern"> graphql:datePattern </TCol>
<TCol> string </TCol><TCol> 日期转换格式 </TCol>
<TCol> 否 </TCol><TCol>

在当前对象属性的数据类型为 `Timestamp`，且系统配置
`nop.graphql.ignore-millis-in-timestamp`（`Timestamp` 类型是否不以毫秒形式返回）为 `true`
时，NopGraphQL 引擎会自动将当前对象属性的值按该设定的形式格式化为字符串后返回。
...
<TRow><TCol id="attr_props_prop_schema_graphql_jsonComponentProp"> graphql:jsonComponentProp </TCol>
<TCol> string </TCol><TCol> </TCol>
<TCol> 否 </TCol><TCol>
...
<TRow><TCol id="attr_props_prop_schema_graphql_dictName"> graphql:dictName </TCol>
<TCol> string </TCol><TCol> </TCol>
<TCol> 否 </TCol><TCol>
...
<TRow><TCol id="attr_props_prop_schema_graphql_dictValueProp"> graphql:dictValueProp </TCol>
<TCol> prop-path </TCol><TCol> </TCol>
<TCol> 否 </TCol><TCol>
...
<TRow><TCol id="attr_props_prop_schema_graphql_authObjName"> graphql:authObjName </TCol>
<TCol> string </TCol><TCol> </TCol>
<TCol> 否 </TCol><TCol>
...
<TRow><TCol id="attr_props_prop_schema_graphql_selection"> graphql:selection </TCol>
<TCol> field-selection </TCol><TCol> </TCol>
<TCol> 否 </TCol><TCol>
...
<TRow><TCol id="attr_props_prop_schema_graphql_transFilter"> graphql:transFilter </TCol>
<TCol> xpl-fn:(filter, query, forEntity) => any </TCol><TCol> 过滤器转换函数 </TCol>
<TCol> 否 </TCol><TCol>

在当前对象属性上做过滤运算的过滤器均会被替换为该转换函数所构造的过滤器，从而简化复杂过滤条件的使用。
...
<TRow><TCol id="attr_props_prop_schema_ui_maskPattern"> ui:maskPattern </TCol>
<TCol> string </TCol><TCol> 掩码模式 </TCol>
<TCol> 否 </TCol><TCol>

GraphQL 返回的数据会自动按照其指定的模式调用 `StringHelper#maskPattern`
函数对当前属性的值做掩码处理，如，`ui:maskPattern="3*4"` 表示保留前 3 位以及后 4 位字符，
中间部分用 `*` 来代替。可应用于安全性要求较高的属性上。其将在 `x:post-extends`
阶段为当前属性自动生成 `transformOut`（详见 Xpl 函数 `meta-gen:GenMaskingExpr`）

</TCol></TRow>

<!-- -->

<TRow><TCol id="attr_props_prop_schema_biz_moduleId"> biz:moduleId </TCol>
<TCol> string </TCol><TCol> 关联对象所属的模块 </TCol>
<TCol> 否 </TCol><TCol>

Nop 模块标识（如，`nop/auth`）。在当前属性关联的是其他模块的对象时，需要通过该属性指示该关联对象所属的模块

</TCol></TRow>

<!-- -->

<TRow><TCol id="attr_props_prop_schema_biz_codeRule"> biz:codeRule </TCol>
<TCol> string </TCol><TCol> 编码规则 </TCol>
<TCol> 否 </TCol><TCol>

用于自动生成订单号、卡号等，该值不为空时，将在 `x:post-extends` 阶段为当前属性自动生成
`autoExpr`（详见 Xpl 函数 `meta-gen:GenCodeRuleAutoExpr`）。
...
<TRow><TCol id="attr_props_prop_schema_ext_relation"> ext:relation </TCol>
...
<TRow><TCol id="attr_props_prop_schema_ext_joinLeftProp"> ext:joinLeftProp </TCol>
...
<TRow><TCol id="attr_props_prop_schema_ext_joinRightProp"> ext:joinRightProp </TCol>
...
<TRow><TCol id="attr_props_prop_schema_ext_joinRightDisplayProp"> ext:joinRightDisplayProp </TCol>
...
<TRow><TCol id="attr_props_prop_schema_orm_manyToManyRefProp"> orm:manyToManyRefProp </TCol>
...
<TRow><TCol id="attr_props_prop_schema_graphql_queryMethod"> graphql:queryMethod </TCol>
<TCol> dict:core/std-query-method / GraphQLQueryMethod </TCol><TCol> 过滤查询名 </TCol>
<TCol> 否 </TCol><TCol>
...
<TRow><TCol id="attr_props_prop_schema_graphql_connectionProp"> graphql:connectionProp </TCol>
<TCol> prop-name </TCol><TCol> 当前 关联查询属性 所指向的关联属性 </TCol>
<TCol> 否 </TCol><TCol>
...
<TRow><TCol id="attr_props_prop_schema_graphql_inputType"> graphql:inputType </TCol>
<TCol> class-name </TCol><TCol> GraphQL 输入参数的类全名 </TCol>
<TCol> 否 </TCol><TCol>

通过 Java 类的成员变量来定义
[GraphQL 输入参数](https://graphql.org/learn/queries/#arguments)，其作用是限定
[Field Selection](https://www.graphql-java.com/documentation/field-selection)
的可用参数并自动进行数据校验和类型转换。最终，自定义的输入参数可以在 [BizModel](/manual/xbiz)
中通过变量 `selection` 获取到（`${selection.args}`）。

例如，类 `GraphQLConnectionInput` 就定义了[关联过滤查询](#relations-connection)的默认输入参数（如
`resourcesList(filter: $filter, limit: 10, offset: 0) { ... }` 中的各项命名参数）。
该类的定义需满足以下条件：
...
<TRow><TCol id="attr_props_prop_schema_graphql_maxFetchSize"> graphql:maxFetchSize </TCol>
<TCol> int </TCol><TCol> 当前 关联查询属性 一次查询所能取的最大数据量 </TCol>
<TCol> 否 </TCol><TCol>

仅对**关联查询属性**有效，用于控制每次查询所能取得的最多数据，
前端指定的分页大小将被限制在该值及其以下。默认为 `-1`，即不受限制

</TCol></TRow>

<!-- -->

<TRow><TCol id="attr_props_prop_schema_graphql_filter"> graphql:filter </TCol>
<TCol> filter-bean </TCol><TCol> 当前 关联查询属性 的默认过滤条件 </TCol>
<TCol> 否 </TCol><TCol>
...
<TRow><TCol id="attr_props_prop_schema_graphql_orderBy"> graphql:orderBy </TCol>
<TCol> OrderFieldBean </TCol><TCol> 当前 关联查询属性 的默认排序条件 </TCol>
<TCol> 否 </TCol><TCol>
...
<TRow><TCol id="attr_props_prop_schema_transformIn"> transformIn </TCol>
...
<TRow><TCol id="attr_props_prop_schema_transformOut"> transformOut </TCol>
<TCol> xpl </TCol><TCol> </TCol>
<TCol> 否 </TCol><TCol>
...
<TRow><TCol id="attr_props_prop_schema_getter"> getter </TCol>
<TCol> xpl </TCol><TCol> </TCol>
<TCol> 否 </TCol><TCol>
...
<TRow><TCol id="attr_props_prop_schema_setter"> setter </TCol>
<TCol> xpl </TCol><TCol> </TCol>
<TCol> 否 </TCol><TCol>
...
<TRow><TCol id="attr_props_prop_schema_auth"> auth </TCol>
...
<TRow><TCol id="attr_props_prop_schema_arg"> arg </TCol>
...
<TRow><TCol id="attr_props_prop_schema_autoExpr"> autoExpr </TCol>
...
<TRow><TCol id="attr_props_prop_schema_"> </TCol>
<TCol> string </TCol><TCol> </TCol>
<TCol> 否 </TCol><TCol>
...
对当前对象的过滤可能会涉及较为复杂的组合条件（如，子查询等），或者是组合条件会出现多次，
