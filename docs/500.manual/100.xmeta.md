...
`io.nop.demo.entity.DepartmentEntity` 的结构：
...
用于设置在查询该 ORM 实体时需始终附加的排序条件。其附加逻辑见 `CrudBizModel#prepareFindPageQuery`。
...
<meta>
  ...
  <selections>
    <selection id="F_moreFields">
      userId, userName, relatedRoleList{ roleId, roleName }
    </selection>
  </selections>
...
在未指定业务对象属性的值时，将采用该缺省值作为其属性值。
值的实际类型将根据 [schema](#attr_props_prop_schema) 确定并自动进行转换。
