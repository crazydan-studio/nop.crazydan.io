...
有关 XMeta 与 NopGraphQL 引擎的更为深入的分析请阅读[《NopGraphQL 引擎》](../implement/component/graphql)。
...
<TCol> xpl </TCol><TCol> <b>当前对象属性</b>的 Getter 函数 </TCol>
<TCol> 否 </TCol><TDesc>

为<b>当前对象属性</b>定义 Getter 函数，其将覆盖对象实体上的 Getter 方法，
以便于添加动态处理逻辑。
...
<TCol> xpl </TCol><TCol> <b>当前对象属性</b>的 Setter 函数 </TCol>
<TCol> 否 </TCol><TDesc>

为<b>当前对象属性</b>定义 Setter 函数，其将覆盖对象实体上的 Setter 方法，
以便于添加动态处理逻辑。

其用于 NopGraphQL 根据客户端提交数据构造<b>当前对象</b>的过程中。
...
- `entity`: 其为<b>当前对象</b>自身；
- `value`: 其为<b>当前对象属性</b>的值。可能为 `null`；
...
用于新增或者修改的时候自动计算<b>当前对象属性</b>的缺省值，
...
用于配置对<b>当前对象属性</b>的读写权限，从而实现对<b>当前对象</b>属性的访问权限控制。
...
<TRow><TCol id="attr_props_prop_autoExpr_when"> when </TCol>
<TCol> csv-set </TCol><TCol> 执行计算的条件 </TCol>
<TCol> 是 </TCol><TDesc>

其为调用 `OrmEntityCopier#copyToEntity` 的 `@BizAction`
的名称列表，以逗号分隔。表示只有在执行该列表内名称的 BizModel Action
时才计算<b>当前对象属性</b>的缺省值。默认的可选值如下：
...
<TRow><TCol id="attr_props_prop_autoExpr_body"> &lt;/> </TCol>
<TCol> xpl </TCol><TCol> 计算函数 </TCol>
<TCol> 是 </TCol><TDesc>
...
- `entity`: 其为<b>当前对象</b>自身；
- `data`: 其为客户端提交的数据；
- `objMeta`: `IObjSchema` 类型，其为<b>当前对象</b>的[结构](#attrs)；
- `propMeta`: `IObjPropMeta` 类型，其为<b>当前对象属性</b>的[结构](#def_attr_props_prop)；
...
- **当前对象属性** _已配置_ [缺省值](#attr_props_prop_defaultValue)；
- **当前对象属性** _已被设置_ 为[虚拟字段](#attr_props_prop_virtual)；

:::tip

[&lt;setter />](#attr_props_prop_setter)
和 [&lt;transformIn />](#attr_props_prop_transformIn)
只在客户端有提交<b>当前对象属性</b>的值时，才会被调用，因此，二者与
`<autoExpr />` 的调用是互斥的，不会同时被调用。

:::

若在<b>当前对象属性</b>上配置了 [biz:codeRule](#attr_props_prop_biz_codeRule)，
...
<TRow><TCol id="attr_props_prop_auth_for"> for </TCol>
<TCol> xml-name </TCol><TCol> 权限类型名 </TCol>
<TCol> 是 </TCol><TDesc>
...
<TRow><TCol id="attr_props_prop_auth_roles"> roles </TCol>
<TCol> csv-set </TCol><TCol> 拥有 for 权限的角色列表 </TCol>
<TCol> 否 </TCol><TDesc>

以逗号分隔的角色列表。若操作者所拥有的角色存在于该列表内，则该操作者便拥有
...
<TRow><TCol id="attr_props_prop_auth_permissions"> permissions </TCol>
<TCol> multi-csv-set </TCol><TCol> 拥有 for 权限的操作权限列表 </TCol>
<TCol> 否 </TCol><TDesc>

以逗号分隔的操作权限列表。若操作者所拥有的操作权限存在于该列表内，则该操作者便拥有
[for](#attr_props_prop_auth_for) 对应的属性访问权限。
...
所有的操作者均可读写<b>当前对象属性</b>。缺省值为 `false`
...
<TRow><TCol id="attr_props_prop_arg_name"> name </TCol>
<TCol> var-name </TCol><TCol> 参数名 </TCol>
<TCol> 是 </TCol><TDesc>
