...
对于非[对象类型](https://graphql.org/learn/schema/#object-types-and-fields)的字段，
不能再对其做字段选择。

> 列表等非[标量类型](https://graphql.org/learn/schema/#scalar-types)的
> GraphQL 字段将使用 `IdentityTypeConverter` 作为类型转换器，
> 其直接返回数据本身，所以，需确保字段取值结果与设定的类型是相同的。相关处理逻辑见
