...
**注意**：`id` 为 NopORM 的保留名字，一般要求不使用 `id` 作为主键属性的名字。但仍然可以通过名字
`id` 得到主键值（调用 `IOrmEntity#get_id`），如果是单一主键，则二者本质是相同的，而如果是复合主键，则通过
`id` 得到的是 `OrmCompositePk` 对象，其包含主键属性及其值，并且该对象的 `toString` 方法返回的是以
`IOrmCompositePk#COMPOSITE_PK_SEPARATOR` 作为分隔符将主键属性值拼接后的结果。NopGraphQL
对于映射了 ORM 实体的业务对象，将会为名字为 `id` 的字段构造取值函数
`OrmEntityIdFetcher` 用于获取单一主键或复合主键的字符串结果，构造逻辑见
`OrmFetcherBuilder#buildFetcher`。
...
其与 Protocol Buffers
中的[字段编号](https://protobuf.dev/reference/protobuf/proto3-spec/#fields)的作用相同，
可以通过该编号识别出对属性名的变更。

在与 ORM 实体映射的[列名](./xorm#attr_entities_entity_columns_column_name)上也会设置相同的
`propId`，在 NopORM 中将根据该编号对属性进行索引，用于为属性赋值等。详见
