...
以下定义的是一个简单的 xmeta 文件，用于描述业务对象 `DepartmentEntity` 的结构：
...
上后保存或更新 `parent` 对象，而 GraphQL 字段 `parentName` 则会取
`parent.name` 的值。
...
ORM 实体对象的哪些属性的值，再根据依赖属性的值获取或组装该属性的值，例如：
...
`userMappings`（为其加前缀 `~` 以标识其为内部属性），并且会在其自定义的取值函数
[&lt;getter />](#attr_props_prop_getter) 中根据依赖属性的值得到其自身的值。

> 设置了 `depends` 的字段将采用取值函数 `OrmDependsPropFetcher`
> 进行取值，其在 `OrmFetcherBuilder#initFetchers` 中确定。

**注意**：依赖属性的列表元素采用 [prop-path](./dict/std-domain#opt_prop_path)
格式，并通过 `,` 分隔。并且，依赖属性必须为 ORM 实体对象属性，但其可以在 XMeta
