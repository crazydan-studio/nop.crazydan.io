...
下面是一个简单的 xmeta 文件，用于描述业务对象 `DepartmentEntity` 的结构，可以窥其全貌：
...
目前仅关注与后端处理相关的 `<prop />` 的属性，对于仅在 XView 层面使用的属性（如
`ui:maxUploadSize` 等）暂时未做整理。

:::

在 XMeta 中是以 `<props />` 标签下的 `<prop />`
节点来描述业务对象的属性信息的，如下所示：
...
> 关于 `<prop />` 的结构定义在 <NopVfsLink module="nop-xdefs" path="/nop/schema/schema/obj-schema.xdef" />
> 中。

在 `<prop />` 内可以完整描述属性的数据类型、必要性、可查询性等，并且可附加与 GraphQL、ORM
等相关的配置信息，为 XMeta 的核心组成部分。下表将详细说明 `<prop />` 节点的配置属性：
...
该属性名也会直接与 ORM
实体的[列名](./xorm#attr_entities_entity_columns_column_name)、GraphQL
的[字段名](../implement/component/graphql#about-graphql)做同名映射，
也即通过名字便可获得属性在 ORM 或 GraphQL 领域模型中的配置信息。

设置的属性名一般为 [prop-name](./dict/std-domain#opt_prop_name)
形式，并且只有该形式的属性才能直接做同名映射。但其也可以是以 `.` 分隔的复合属性，如
`parent.name`，其主要用于显式声明是否可对关联对象 `parent` 的 `name`
属性做增（`insertable`）改（`updatable`）查（`queryable`）或排序（`sortable`）：
...
为了安全性，NopGraphQL 默认不允许对关联对象的属性直接做增改查等操作。
若有需要，则只能像上例那样，除了定义关联对象属性 `parent` 以外，还必须定义相应的复合属性，如
`parent.name`，并在该复合属性上配置启用或禁用
[queryable](#attr_props_prop_queryable)、[updatable](#attr_props_prop_updatable)
等。

反言之，若是没有对关联对象的属性做增改查的需求，则不需要定义复合结构的属性。

**注意**：复合属性不能作为 GraphQL 的选择字段。相关处理逻辑见
...
<TCol> ISchema </TCol><TCol> 属性类型的模式 </TCol>
<TCol> 否 </TCol><TDesc>

用于设置属性的数据类型、数据精度等结构和约束信息，如：
...
用以保证数据转换和校验的可推导性与有效性。
详细说明见参考手册[《基础 DSL：Schema》](./dsl/schema#attrs)。
...
<TCol> string </TCol><TCol> 属性的描述信息 </TCol>
<TCol> 否 </TCol><TDesc>

用于描述说明属性的使用和注意事项等内容，如：
...
**[ORM Only]** 用于设置属性的缺省值，如：

```xml
    <prop name="name" defaultValue="匿名" />
```

在调用 GraphQL 变更函数 `CrudBizModel#save` 做 ORM 实体新增时，
若客户端未指定属性值并且该属性也未配置 [autoExpr](#attr_props_prop_autoExpr)，则在
`AutoExprRunner#runAutoExpr` 中会为新增实体的属性设置该缺省值。
...
> 可通过配置项 `nop.grpc.auto-init-prop-id` 设置是否自动生成属性编号，缺省为 `true`。

在与 ORM 实体的列相映射时，该属性编号与列的
[`propId`](./xorm#attr_entities_entity_columns_column_propId)
配置相同。NopORM 将根据该编号对 ORM 实体属性进行索引，用于为属性赋值等。

> 对 ORM 属性赋值的逻辑见 `DynamicOrmEntity#orm_propValue(int, java.lang.Object)`。

属性编号的值必须大于 `0`，并且与 ORM 实体列相映射时，该值不能超过
`OrmModelConstants#MAX_PROP_ID`（固定为 `2000`），而与
gRPC 字段映射时，该值没有上限约定。

**注意**：在与 NopORM、gRPC 的列或字段映射之外的场景，不需要配置该属性。
...
</TCol><TCol> 属性所映射到的 ORM 实体属性名 </TCol>
<TCol> 否 </TCol><TDesc>

**[ORM Only]** 用于为 ORM 实体的属性定义一个别名，例如：

```xml
    <prop name="parentName" mapToProp="parent.name" />
```

其表示为 ORM 实体的 `parent.name` 属性定义别名 `parentName`，
在客户端均由 `parentName` 指代 `parent.name`。
...
> 在过滤条件中的别名将在 `BizQueryHelper#transformMapToProp`
> 中被替换为目标 ORM 实体属性 `parent.name`。

再比如，通过别名 `parentName` 做 ORM 实体对象的增改，并以别名做 GraphQL 字段选择：
...
最终，`$data` 变量中的别名 `parentName` 的值将在赋值到 `parent.name`
上后保存 `parent` 对象，而 GraphQL 字段 `parentName` 则会取
`parent.name` 的值。

> 在增改 ORM 实体对象时，调用 `OrmEntityCopier#copyField`
> 会将别名的值复制到 `parent.name` 上。而在做 GraphQL 字段选择时，则会通过
> `OrmFetcherBuilder#initFetchers` 为别名字段绑定的取值函数
> `OrmDependsPropFetcher` 取出 `parent.name` 的值。
...
</TCol><TCol> 属性所依赖的 ORM 实体属性列表 </TCol>
<TCol> 否 </TCol><TDesc>

**[ORM Only]** 用于设置在对属性做 GraphQL 字段取值时，需先获取
ORM 实体对象的哪些关联属性的值，再根据依赖属性的值获取或组装该属性的值，例如：
...
`userMappings`，并且会在其自定义的取值函数 [&lt;getter />](#attr_props_prop_getter)
中根据依赖属性的值得到其自身的值。

> 设置了 `depends` 的字段将采用取值函数 `OrmDependsPropFetcher`
> 进行取值，其在 `OrmFetcherBuilder#initFetchers` 中确定。

依赖属性的列表元素采用 [prop-path](./dict/std-domain#opt_prop_path)
格式，并通过 `,` 分隔。依赖属性必须为 ORM 实体对象的属性，但其可以在 XMeta
中有对应的映射属性，也可以没有，若未定义相应的 XMeta 属性，则在属性名前面需添加前缀
`~`，如 `~userMappings`，用以表示其为内部属性。

**注意**：若在 `<getter />` 中引用的不是 ORM 实体对象的关联属性，或者引用的属性未与
ORM 实体对象的属性相映射，则无需配置为依赖属性，直接通过 `entity` 引用即可。
...
若设置其值为 `true`，则当前属性的值不能为空值（`null` 或空字符串）。缺省为 `false`。

在对 ORM 实体对象做新增和更新时，必须为必填属性赋非空值，
...
若设置其值为 `true`，则表示当前属性为内部属性。缺省为 `false`。

内部属性在 XView 生成的页面中默认不会为其创建对应的组件。
但并不影响该属性作为 GraphQL 的选择字段，也不影响对该属性做
ORM 新增或更新操作。

> 仅 [published](#attr_props_prop_published) 才会影响属性是否可作为
> GraphQL 的选择字段。
...
若设置其值为 `true`，则表示当前属性已被废弃，已废弃的属性将不再被使用。缺省为 `false`
...
若设置其值为 `true`，则表示在新增业务对象时，可以插入当前属性的值，
否则，当前属性的值将被忽略。缺省为 `false`。
...
若设置其值为 `true`，则表示在更新业务对象时，可以更新当前属性的值，
否则，当前属性的值将被忽略。缺省为 `false`。
...
若设置其值为 `true`，则在新增、更新 ORM 实体对象时，当前属性的值不会被复制到
...
若设置为 `false`，则当前属性不能作为 GraphQL 选择字段出现在
...
只有在将其设置为 `true` 且 [published](#attr_props_prop_published)
也为 `true` 时，才可以导出当前属性的值。缺省为 `true`。
...
变量中指定根据当前属性做数据排序，否则，将抛出属性不可排序的异常
...
只有将其设置为 `true`，才能够在 GraphQL 服务调用时，在 `$query`
变量中使用当前属性构造过滤条件，否则，将抛出属性不可被用于查询条件的异常
...
**注意**：对于作为[过滤器转换](#def_attr_props_prop_graphql_transFilter)的属性，
则需要强制设置该配置项为 `true`。
...
用于标记当前属性是否为懒加载的。缺省为 `false`。

若当前属性映射的是 ORM 实体对象的[关联属性](./xorm#def_attr_entities_entity_relations)，
则该配置项应始终为 `true`。
