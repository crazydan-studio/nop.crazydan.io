...
| [schema](#def_attr_props_prop_schema) | `ISchema` | 属性 Schema | 否 | 对当前属性值类型、值精度等的约束定义 |
...
| queryable | boolean | 是否可查询？ | 是 | 该属性对应的 ORM 表字段是否可以被作为查询字段出现在 SQL 的 `where` 语句中，与 `allowFilterOp` 配合，可限制在该属性对应的 ORM 表字段上能够应用的过滤运算。默认为 `false`。注：作为[转换过滤器](#def_attr_props_prop_graphql_transFilter)的对象属性也需要设置该属性值为 `true` |
...
| graphql:transFilter | `xpl-fn:(filter, query, forEntity) => any` | 过滤器的转换函数 | 否 | 通过在当前对象属性上做过滤运算，以屏蔽子查询等复杂过滤器及其实现细节，从而简化过滤查询条件的设置。详细说明见 [XMeta 过滤器转换](#def_attr_props_prop_graphql_transFilter) |
...
在 XMeta 中直接定义一个配置了过滤器转换函数 `graphql:transFilter`
