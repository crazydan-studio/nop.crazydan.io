...
- `/html/body/div/select`: 以**绝对路径**定位目标节点；
- `div/select`: 以**相对路径**定位目标节点，其从**当前节点**往下查找；
- `//div//p`: **任意层级**中的目标节点，其中，`//` 表示从**当前节点**往下的任意层级；
- `div/*/input`: 以通配符 `*` 匹配该层级中的任意兄弟节点；
- `div[@attr='west']`: 选择属性名为 `attr` 且其值为 `west` 的 `div` 节点；
- `div[@attr]`: 选择拥有名为 `attr` 属性的 `div` 节点；
- `.`: 选择当前节点；
- `..`: 选择父节点；
- `p[2]`: 选择序号为 `2` 的 `<p/>` 子节点。其中，序号从 `0` 开始；
- `*[2]`: 选择序号为 `2` 的任意子节点。其中，序号从 `0` 开始；
- `#p`: 选择**第一个** `p` 节点；
- `#p[@attr]`: 选择**第一个**拥有名为 `attr` 属性的 `p` 节点；
- `div/:[@attr]`: TODO 暂不明确；
- `div/@attr`: 选择 `div` 节点中名为 `attr` 的属性的值；
- `div/$value`: 选择 `div` 节点值，其对应的是 [xdef:value](../xlang/xdef#attr_xdef_value)
  所指定的节点内容；
- `div/$innerXml`: 选择 `div` 节点内的 xml 内容（字符串）；
- `div/$xml`: 选择 `div` 节点自身的 xml 内容（字符串）；
- `div/$tag`: 选择 `div` 节点的标签名；
- `div/$text`: 选择 `div` 节点中的文本内容（包含各级子节点的文本内容）；
