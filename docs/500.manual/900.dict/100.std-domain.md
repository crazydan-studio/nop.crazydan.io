...
成员变量也将被定义为数据域所对应的「目标数据类型」，而在解析 DSL 时，
也会自动根据数据域的约束对相应的属性值进行解析和校验，从而确保 DSL 模型的完整性和有效性：
...
### def-type {#opt_def_type}

<Table head={['名称', '目标数据类型']}>

<TRow><TCol> def-type </TCol><TCol>

`io.nop.xlang.xdef.XDefTypeDecl`

</TCol><TDesc>

一般在**元元模型**中设定，用于支持为**元模型**中的相应属性指定不同的数据域，
也就是，其描述的是 DSL 属性的属性类型。

其内容格式为 `(!~#)?{stdDomain}(:{options})?(={defaultValue})?`，如
`!dict:core/some-dict=dict1`。

> 针对该格式的详细说明见 [XDef 属性类型](../xlang/xdef#attr_type)。

其中，`stdDomain` 可以为包含 `def-type` 自身在内的所有注册到
`StdDomainRegistry` 中的数据域的名字：

```xml title="/nop/schema/xdef.xdef"
  <!-- ... -->
  <meta:define ...
...
**注**：其由 `SimpleStdDomainHandlers.DefTypeType` 对源数据做类型转换。

</TDesc></TRow>

</Table>

### xdef-attr {#opt_xdef_attr}

<Table head={['名称', '目标数据类型']}>

<TRow><TCol> xdef-attr </TCol><TCol>
...
当前仅用于元元模型 `/nop/schema/xdef.xdef` 中，以对其自身进行**交叉定义**：
...
其中，`meta:unknown-attr="!xdef-attr"` 将用于描述所有的
`xdef` 名字空间的属性，且这些属性的值均为 [def-type](#opt_def_type) 类型。

所谓交叉定义就是，`meta:unknown-attr="!xdef-attr"`
定义了 `xdef:unknown-attr="def-type"`，而 `xdef:unknown-attr="def-type"`
又反过来定义了 `meta:unknown-attr="!xdef-attr"`。

**注**：其由 `SimpleStdDomainHandlers.XdefAttrType` 对源数据做类型转换。
...
在从 DSL 到 Java 对象的解析过程中，将会调用枚举类中的该方法将 DSL
中的文本属性值转换为对应的枚举项。
...
其仅在从 DSL 到 Java 对象的解析过程中，通过 `RegexHelper#compileRegex`
对该类型的值进行有效性检查，并不会将其解析为相应的正则表达式对象，其类型始终为 `String`。

可通过 `RegexHelper#registerRegexCompiler` 注册所需的正则表达式编译器
`IRegexCompiler`，对 `regex` 类型的格式也由该编译器决定。Nop 采用
`JdkRegexCompiler` 作为缺省的正则表达式编译器，也即，该类型的数据格式由
`java.util.regex.Pattern` 决定。
...
### xdef-ref {#opt_xdef_ref}

<Table head={['名称', '目标数据类型']}>

<TRow><TCol> xdef-ref </TCol><TCol>

`java.lang.String`

</TCol><TDesc>

用于引用当前 XDef 中的 xdef 命名片段（由 [xdef:name](../xlang/xdef#attr_xdef_name) 指定其名字）：
...
> [xdef:ref](../xlang/xdef#attr_xdef_ref) 在 `/nop/schema/xdef.xdef`
> 中的定义为 `xdef:ref="xdef-ref"`。

该类型也可以用于引用外部元模型文件：

```xml title="/nop/schema/query/query.xdef"
...
**注**：其由 `SimpleStdDomainHandlers.XDefRefType` 对源数据做类型转换。

</TDesc></TRow>

</Table>

### method-ref {#opt_method_ref}

<Table head={['名称', '目标数据类型']}>

<TRow><TCol> method-ref </TCol><TCol>

`io.nop.xlang.expr.MethodRef`

</TCol><TDesc>

对 Java class 方法的引用。其格式为 `{ownerName}::{methodName}`，其中，`{ownerName}`
为 Java class 全类名，`{methodName}` 为该类中的方法名，如
`io.nop.xlang.expr.MethodRef::parse`。

**注**：其由 `SimpleStdDomainHandlers.MethodRefType` 对源数据做类型转换。
...
在 DSL 中，对于该类型的属性，可按需填写包括 `std-domain`
自身在内的任一注册到 `StdDomainRegistry` 中的数据域的名字：

```xml {5} title="/nop/schema/xlib.xdef"
<lib ...>
  <tags ...>
    <xdef:unknown-tag ...>
      <attr name="!xml-name"
...
```xml {7} title="/nop/web/xlib/web.xlib"
...
**注**：其由 `SimpleStdDomainHandlers.StdDomainType` 对源数据做类型转换。
...
**注**：其由 `SimpleStdDomainHandlers.StdType` 对源数据做类型转换。
...
**注**：其由 `SimpleStdDomainHandlers.StdSqlTypeType` 对源数据做类型转换。
...
**注**：其由 `SimpleStdDomainHandlers.StdSqlTypeListType` 对源数据做类型转换。
...
如果是以 [xdef:value](../xlang/xdef#attr_xdef_value) 指定节点内容的类型为
`eval-code`，则可以在该节点中使用 Xpl 标签函数：
...
> 若 `xpl:outputMode` 为 `text`、`xml` 等文本输出模式，则需调用
> `EvalCode#generateText` 才能得到结果。
...
**注**：支持在 [<x:config/>](../xlang/xdsl#tag_x_config) 中统一导入
Java class 或 xlib 标签库。

**注**：其实现逻辑见 `XplStdDomainHandlers.EvalCodeType`。
...
<TRow><TCol> expr </TCol><TCol>

`io.nop.core.lang.eval.IEvalAction`（实例类型为 `io.nop.xlang.api.ExprEvalAction`）

</TCol><TDesc>

可得到计算结果的 Xpl 表达式，如 `a > 1`、`b == 1`，可以为
`a > 1 && b < 2` 这样的逻辑组合，也可以是对变量的取值 `user.name`，但不能是
`a = b` 这样的赋值语句。
...
**注**：可以使用 `and` 代替 `&&`，或使用 `or` 代替 `||`。

**注**：其实现逻辑见 `XplStdDomainHandlers.ExprType`。
...
<TRow><TCol> s-expr </TCol><TCol>

`io.nop.core.lang.eval.IEvalAction`（实例类型为 `io.nop.xlang.api.ExprEvalAction`）

</TCol><TDesc>

通过 `${}` 包裹的单个 [expr](#opt_expr)，如 `${ a > 1}`、`${ a > 1 && b < 2 }`。
...
**注**：其实现逻辑见 `XplStdDomainHandlers.SingleExprType`。
...
<TRow><TCol> t-expr </TCol><TCol>

`io.nop.core.lang.eval.IEvalAction`（实例类型为 `io.nop.xlang.api.ExprEvalAction`）

</TCol><TDesc>

在文本中嵌入零个或多个 [s-expr](#opt_s_expr) 的 Xpl 模版，如
`a = ${ a }, b > 1 is ${ b > 1 }`。
...
**注**：其实现逻辑见 `XplStdDomainHandlers.TplExprType`。
...
**注**：`%{}` 用于隔离 XNode transform 阶段的表达式内容。

**注**：其实现逻辑见 `XplStdDomainHandlers.XtExprType`。

</TDesc></TRow>

</Table>

### xt-value {#opt_xt_value}

<Table head={['名称', '目标数据类型']}>

<TRow><TCol> xt-value </TCol><TCol>

`io.nop.core.lang.eval.IEvalAction`（实例类型为 `io.nop.xlang.api.ExprEvalAction`）

</TCol><TDesc>

在文本中嵌入零个或多个 [xt-expr](#opt_xt_expr) 的 Xpl 模版，如
`a = %{ a }, b > 1 is %{ b > 1 }`。
...
**注**：其实现逻辑见 `XplStdDomainHandlers.XtValueType`。

</TDesc></TRow>

</Table>

### flags-expr {#opt_flags_expr}

<Table head={['名称', '目标数据类型']}>

<TRow><TCol> flags-expr </TCol><TCol>
...
也即，若表达式中的某个标记存在于 `enabledFlags` 列表中，则其表示
`true`，否则其代表 `false`，最后计算得到整个表达式的结果为 `true` 或 `false`。

在该表达式中，可以使用 `EMPTY` 表示匹配**空标记列表**，如
`a || EMPTY`，或者使用 `ANY` 表示匹配**非空**列表中的任意标记。

**注**：可以使用 `and` 代替 `&&`，`or` 代替 `||`，`not` 代替 `!`。

**注**：其实现逻辑见 `SimpleStdDomainHandlers.FlagsExprType`。

</TDesc></TRow>

</Table>

### t-report-expr {#opt_t_report_expr}

<Table head={['名称', '目标数据类型']}>

<TRow><TCol> t-report-expr </TCol><TCol>

`io.nop.core.lang.eval.IEvalAction`（实例类型为 `io.nop.xlang.api.ExprEvalAction`）

</TCol><TDesc>

与 [t-expr](#opt_t_expr) 相同，只是其在 `t-expr` 的基础上增加了对 Excel 公式的支持，如
`SUM(A3:A5) + C1`，同时提供与 Excel 相关的函数，如 `SUM`、`PRODUCT` 等。

**注**：Excel 函数由 `ReportFunctions` 提供实现，被 `@Description` 标注的函数均可调用。

**注**：其实现逻辑见 `TemplateReportExprStdDomainHandler`。
...
XPath 选择器。其缺省由 `DefaultXPathProvider#compile` 通过
`XPathHelper#parseXSelector` 解析得到。

> 可通过 `XPathProvider#registerInstance` 替换缺省实现。

其支持以下 XPath 选择器规则：

- `/html/body/div/select`: 以**绝对路径**查找目标节点；
- `div/select`: 以**相对路径**查找目标节点，其从**当前节点**往下搜索；
- `//div//p`: 查找**任意层级**中的目标节点，其中，`//` 表示从**当前节点**往下的任意层级；
- `..`: 选择父节点；
- `*`: 以通配符 `*` 匹配其所在层级中的任意兄弟节点；
- `p[2]`: 选择序号为 `2` 的 `<p/>` 子节点。其中，序号从 `0` 开始；
- `div[expr]`: 选择在节点上执行的 `expr` 表达式结果为 `true` 的 `div` 节点。
  在表达式中，`@属性名` 的形式表示取节点上的属性值，
  因此，可以在 `[]` 中根据属性值做判断，如 `[ @name != 'Lily' && @age > 20 ]`；
-
- `div/:[expr]`: 获得在 `div` 节点上执行 `expr` 表达式的执行结果。
  在表达式中，`@属性名` 的形式表示取节点上的属性值，
  因此，可以在 `:[]` 中编写复杂表达式，如 `:[ 'Gender is ' + (@gender == '1' ? 'female' : 'male') ]`；
- `div/@attr`: 获得 `div` 节点中名为 `attr` 的属性的值。
  与 `div/:[expr]` 不同的是，其为静态取值，而后者为动态取值；
- `div/$value`: 获得 `div` 节点值，其对应的是 [xdef:value](../xlang/xdef#attr_xdef_value)
  所指定的节点内容；
...
**注**：其由 `SimpleStdDomainHandlers.XPathType` 对源数据做类型转换。

**注**：可通过 `XPathOperatorRegistry#registerOperator` 注册自定义的
`$xx` 形式的值选择器。
...
### xjson-list {#opt_xjson_list}

<Table head={['名称', '目标数据类型']}>

<TRow><TCol> xjson-list </TCol><TCol>
...
### xjson-map {#opt_xjson_map}

<Table head={['名称', '目标数据类型']}>

<TRow><TCol> xjson-map </TCol><TCol>
...
### xjson-node {#opt_xjson_node}

<Table head={['名称', '目标数据类型']}>

<TRow><TCol> xjson-node </TCol><TCol>
...
### xml-body {#opt_xml_body}

<Table head={['名称', '目标数据类型']}>

<TRow><TCol> xml-body </TCol><TCol>
