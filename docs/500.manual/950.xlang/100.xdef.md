...
在 XDef 文件中的所有属性的值都是 [def-type](../dict/std-domain#opt_def_type)
...
- `{stdDomain}` 为在 `StdDomainRegistry` 中注册的数据域的名字，如
  [dict](../dict/std-domain#opt_dict)，具体值详见以 `XDefConstants#STD_DOMAIN_`
  开头的常量，使用手册见[《标准数据域》](../dict/std-domain)
...
则需要相应地为该自定义类型[注册类型转换器](#appendix_register_type_converter)，否则，
...
为 xdef 片段 [<xdef:define/>](#tag_xdef_define) 命名，其他节点可以通过
[xdef:ref](#attr_xdef_ref) 来引用该片段。若是在其他节点中，则其将作为 Java
类名，在[生成节点的 class 模型](#appendix_generate_node_class)时会根据它和根节点上的
[xdef:bean-package](#attr_xdef_bean_package) 设置自动生成
[xdef:bean-class](#attr_xdef_bean_class) 属性，并进而创建对应的 class 文件。
...
引用相当于是继承已有定义（[生成的 class 模型](#appendix_generate_node_class)采用
`extends` 派生）。如果再增加属性或者子节点则表示在已有定义基础上扩展：

```xml {2,7} title="/nop/schema/wf/wf.xdef"
<workflow ...>
  <xdef:define xdef:name="WorkflowStepModel" ...>
    ...
  </xdef:define>
...
```xml {5-7} title="/nop/schema/query/query.xdef"
...
**注意**：通过 [x:extends](./xdsl#attr_x_extends) 可做差量，而
`xdef:ref` 只能做增量。
...
**注意**：若在该节点中定义了多类子节点（`union` 类型不管子节点数量），则必须配置
[xdef:bean-sub-type-prop](#attr_xdef_bean_sub_type_prop)
或 [xdef:bean-tag-prop](#attr_xdef_bean_tag_prop)，
用以在其指定的扩展属性（如上例中的 `type`）上记录标签名，确保可将模型对象转换为 `XNode`。
...
### xdef:value {#attr_xdef_value}

<Doc token="xdef:value" href="#attr_xdef_value">

<Table head={['名称', '类型', '必填？']}>

<!-- -->

<TRow><TCol> xdef:value </TCol><TCol>
...
**[仅根节点]** 指定根节点及其子节点所[生成的 class 模型](#appendix_generate_node_class)的包名。

**注意**：只要配置了 `xdef:bean-package`，则所有配置了 [xdef:name](#attr_xdef_name)
的节点（包括 [<xdef:define/>](#tag_xdef_define)）均会生成 class 模型。
...
**注意**：若是未设置 [xdef:name](#attr_xdef_name)，即不为该节点生成 class 模型，则不能为其配置
`xdef:bean-body-prop`，否则，在生成根节点的 class 模型时将会报异常
`params={expr=prop.schema!.type!},desc=值不允许为null`。
...
**注意**：对于配置了 [xdef:body-type](#attr_xdef_body_type)
的节点，若其定义了多类子节点（`union` 类型不管子节点数量），则必须配置
[xdef:bean-sub-type-prop](#attr_xdef_bean_sub_type_prop) 或 `xdef:bean-tag-prop`，
用以在其指定的扩展属性（如上例中的 `type`）上记录标签名，确保可将模型对象转换为 `XNode`。

**注意**：若是需要在子节点对应的**模型对象**属性上记录标签名，则必须在子节点上配置
`xdef:bean-tag-prop`。
...
**注意**：对于配置了 [xdef:body-type](#attr_xdef_body_type)
的节点，若其定义了多类子节点（`union` 类型不管子节点数量），则必须配置
`xdef:bean-sub-type-prop` 或 [xdef:bean-tag-prop](#attr_xdef_bean_tag_prop)，
用以在其指定的扩展属性（如上例中的 `type`）上记录标签名，确保可将模型对象转换为 `XNode`。
...
**注意**：仅在直接以 `xdef:pre-parse` 所在 `*.xdef` 生成模型对象时有效，在以
[xdef:ref](#attr_xdef_ref) 引入的元模型中无效。
...
**注意**：仅在直接以 `xdef:post-parse` 所在 `*.xdef` 生成模型对象时有效，在以
[xdef:ref](#attr_xdef_ref) 引入的元模型中无效。
...
**注意**：该脚本的最后一条赋值或 `return` 语句的结果会成为
`DslModelHelper#parseDslNode` 的返回结果。
...
### 注册类型转换器 {#appendix_register_type_converter}

类型转换器 `ITypeConverter` 的作用是在调用 `IBeanModel#setProperty`
动态为模型属性赋值时，自动将赋值参数转换为 `setter` 接口的参数类型，
从而保证数据转换的准确性。

假设[注册了一个自定义类型](#appendix_register_and_use_attr_type) `XuiSize`：
